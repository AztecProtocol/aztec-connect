...
std::shared_ptr<proving_key> MiMCComposer::compute_proving_key()
{
    ...
    size_t log2_n = static_cast<size_t>(log2(n + 1 + offset));
    if ((1UL << log2_n) != (n + 1)) {
        ++log2_n;
    }
    ...
    for (size_t i = 0; i < new_n; ++i) {
        ...
        poly_q_3[i] = q_3[i];
        poly_q_c[i] = q_c[i];
        poly_q_mimc_coefficient[i] = q_mimc_coefficient[i];
        poly_q_mimc_selector[i] = q_mimc_selector[i];
    }
...
std::shared_ptr<verification_key> MiMCComposer::compute_verification_key()
{
    ...
    std::array<fr::field_t*, 10> poly_coefficients;
