 ...
 * The last product term we can compute on-the-fly, as it grows by one element for each additional inverse that we require.
 * 
...
void add_affine_points(g1::affine_element* points, const size_t num_points, fq::field_t* scratch_space)
{
    ...
    // {
    //     std::cout << "first point addition segment: points = " << num_points << " : " << diff.count() << "us" << std::endl;
    // }
    ...
    // if (num_points > 100000)
    // {
    //     std::cout << "second point addition segment: points = " << num_points << " : " << diff.count() << "us" << std::endl;
    // }
...
g1::affine_element* reduce_buckets(affine_product_runtime_state& state, bool first_round)
{
   ...
   // time_start = std::chrono::steady_clock::now();
    ...
    for (size_t i = 0; i < max_bucket_bits; ++i) {
        const uint32_t points_in_round = (static_cast<uint32_t>(state.num_points) - state.bit_offsets[i + 1]) >> static_cast<uint32_t>(i);
