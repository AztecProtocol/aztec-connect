 ...
 * (for a point P \in \G === (x, y) \in \Fq, then (\beta x, y) = (\lambda) * P , where \beta = 1^{1/3} mod Fq and \lambda = 1^{1/3} mod Fr)
 * (which means we can represent a scalar multiplication (k * P) as (k1 * P + k2 * \lambda * P), where k1, k2 have 127 bits)
 * (see field::split_into_endomorphism_scalars for more details)
 * 
 * Once we have our 127-bit scalar multipliers, we determine the optimal number of pippenger rounds, given the number of points we're multiplying.
 * Once we have the number of rounds, `m`, we need to split our scalar into `m` bit-slices. Each pippenger round will work on one bit-slice.
 * 
 * Pippenger's algorithm works by, for each round, iterating over the points we're multplying. For each point, we examing the 
 * point's scalar multiplier and extract the bit-slice associated with the current pippenger round (we start with the most significant slice). We then use the bit-slice to index a 'bucket',
 * which we add the point into. For example, if the bit slice is 01101, we add the corresponding point into bucket[13].
 * 
 * At the end of each pippenger round we concatenate the buckets together. E.g. if we have 8 buckets, we compute:
 * sum = bucket[0] + 2 * bucket[1] + 3 * bucket[2] + 4 * bucket[3] + 5 * bucket[4] + 6 * bucket[5] + 7 * bucket[6] + 8 * bucket[7].
 * 
 * At the end of each pippenger round, the bucket sum will contain the scalar multiplication result for one bit slice.
 * For example, say we have 16 rounds, where each bit slice contains 8 bits (8 * 16 = 128, enough to represent our 127 bit scalars).
 * At the end of the first round, we will have taken the 8 most significant bits from every scalar multiplier.
 * Our bucket sum will be the result of a mini-scalar-multiplication, where we have multiplied every point by the 8 most significant bits
 * of each point's scalar multiplier.
 * 
 * We repeat this process for every pippenger round. In our example, this gives us 16 bucket sums.
 * We need to multiply the most significant bucket sum by 2^{120}, the second most significant bucket sum by 2^{112} etc.
 * Once this is done we can add the bucket sums together, to evaluate our scalar multiplication result.
 * 
 * Pippenger has complexity O(n / logn), because of two factors at play: the number of buckets we need to concatenate per round,
 * and the number of points we need to add into buckets per round.
 * 
 * To minimize the number of point additions per round, we want fewer rounds. But fewer rounds increases the number of bucket concatenations.
 * The more points we have, the greater the time saving when reducing the number of rounds, which means we can afford to have more buckets per round.
 * 
 * For a concrete example, with 2^20 points, the sweet spot is 2^15 buckets - with 2^15 buckets we can evaluate our 127 bit scalar multipliers
 * in 8 rounds (we can represent b-bit windows with 2^{b-1} buckets, more on that below).
 * 
 * This means that, for each round, we add 2^21 points into buckets (we've split our scalar multpliers into two half-width multipliers, so
 * each round has twice the number of points. This is the reason why the endormorphism is useful here; without the endomorphism, we would
 * need twice the number of buckets for each round).
 * 
 ...
 * (each scalar multiplier has ~2^7 non-zero bits, and there are 2^20 points).
 * 
 * This makes pippenger 8 times faster than the naive O(n) equivalent. Given that a circuit with 1 million gates will require
 * 9 multiple-scalar-multiplications with 2^20 points, efficiently using Pippenger's algorithm is essential for fast provers
 * 
 * One additional efficiency gain is the use of 2^{b-1} buckets to represent b bits. To do this we represent our bit-slices in non-adjacent form.
 *  Non-adjacent form represents values using a base, where each 'bit'
 * can take the values (-1, 0, 1). This is considerably more efficient than binary form for scalar multiplication, as inverting a point can be
 * done by negating the y-coordinate.
 * 
 * We actually use a slightly different representation than simple non-adjacent form. To represent b bits, a bit slice contains values
 * from (-2^{b} - 1, ..., -1, 1, ..., 2^{b} - 1). i.e. we only have odd values. We do this to eliminate 0-valued windows, as having a conditional
 * branch in our hot loop to check if an entry is 0 is somethin we want to avoid.
 * 
 * The above representation can be used to represent any binary number as long as we add a 'skew' factor. Each scalar multiplier's `skew`
 * tracks if the scalar multiplier is even or odd. If it's even, `skew = true`, and we add `1` to our multiplier to make it odd.
 * 
 * We then, at the end of the Pippenger algorithm, subtract a point from the total result, if that point's skew is `true`.
 * 
...
inline void compute_wnaf_states(multiplication_runtime_state& state, fr::field_t* scalars)
{
    ...
    // writing to newly allocated memory. The page faults were adding up to 200 milliseconds onto the runtime of our algorithm!
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < num_threads; ++i)
    {
        ...
        size_t offset = i * num_points_per_thread;
        ...
        // If we collect 8 wnaf entries together, we ensure that multiple iterations will not require the same cache lines
        // (wnaf entry = 8 bytes => 8 entries = 64 bytes)
        for (size_t j = 0; j < num_initial_points_per_thread; j += 4)
        {
            ...
            wnaf::fixed_wnaf_packed<bits_per_bucket + 1>(
                &T0.data[0], &wnaf_entries[0], skew_table[j << 1UL], ((j << 1UL) + offset) << 32UL);
            wnaf::fixed_wnaf_packed<bits_per_bucket + 1>(
 ...
 **/ 
template <size_t num_points> inline void organize_buckets(multiplication_runtime_state& state)
{
    constexpr size_t num_rounds = get_num_rounds(num_points);
#ifndef NO_MULTITHREADING
#pragma omp parallel for
