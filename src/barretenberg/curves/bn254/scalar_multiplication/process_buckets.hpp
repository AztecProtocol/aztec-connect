...
{
template <size_t num_bits, size_t shift>
inline void radix_sort(uint64_t* keys, const size_t num_entries) noexcept
{
    ...
    for (size_t i = 0; i < num_buckets; ++i)
    {
        ...
        while (bucket_start != bucket_end)
        {
            ...
            {
                const uint64_t value = (*it >> shift) & mask;
...
template <size_t num_entries, size_t num_bits>
inline void process_buckets(uint64_t* wnaf_entries) noexcept
{
