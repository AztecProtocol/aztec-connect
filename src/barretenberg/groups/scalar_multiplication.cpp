...
std::vector<g1::affine_element*> generate_pippenger_precompute_table(g1::affine_element *points, g1::affine_element *table, size_t num_points, size_t bits_per_bucket)
{
...
void compute_wnaf_state(multiplication_runtime_state &state, wnaf_runtime_state &wnaf_state, fr::field_t *scalars, size_t num_initial_points, fr::field_t *endo_scalars, size_t forced_bucket_width)
{
    size_t bits_per_bucket = (forced_bucket_width > 0) ? forced_bucket_width : get_optimal_bucket_width(num_initial_points);
...
g1::element alt_pippenger_internal(fr::field_t *scalars, g1::affine_element *points, size_t num_initial_points, fr::field_t *endo_scalars, size_t forced_bucket_width)
{
    ...
    for (size_t i = 0; i < state.num_rounds; ++i)
    {
        wnaf_buckets.emplace_back(std::vector<std::vector<uint32_t> >(static_cast<uint32_t>(1U << bits_per_bucket)));
                    ...
                    // g1::conditional_negate_affine(&points[point_data & 0x0fffffffU], &state.addition_temporary, static_cast<uint64_t>((point_data >> 31) & 1));
                    // g1::mixed_add(state.buckets[j], state.addition_temporary, state.buckets[j]);
    ...
    bucket_add_data* bucket_data = (bucket_add_data*)(aligned_alloc(32, state.num_points * state.num_rounds * (sizeof(bucket_add_data))));
    ...
    for (size_t i = 0; i < wnaf_buckets.size(); ++i)
    {
        ...
        for (size_t j = 0; j < wnaf_buckets[i].size(); ++j)
        {
            for (size_t k = 0; k < wnaf_buckets[i][j].size(); ++k)
            {
                ...
                ++count;
                // bucket_data.push_back({ &points[point_data & 0x0fffffffU], &state.buckets[j], static_cast<bool>(point_data >> 31) });
            }
        }
    }
    // bucket_data.push_back(bucket_data.back()); // dummy value so we can fetch data from the 'next bucket' without hitting a bounds error
    printf("c\n");
    for (size_t i = 0; i < state.num_rounds; ++i)
    {
       // handle 0 as special case
        ...
        for (size_t j = start; j < end; ++j)
        {
            ...
            g1::conditional_negate_affine(&points[bucket_data[j].point_data & 0x0fffffffU], &state.addition_temporary, static_cast<uint64_t>(bucket_data[j].point_data >> 31U));
...
g1::element alt_pippenger(fr::field_t *scalars, g1::affine_element *points, size_t num_initial_points, size_t forced_bucket_width) 
{
    ...
    g1::element res = alt_pippenger_internal(endo_scalars, points, num_initial_points, endo_scalars, forced_bucket_width);
    free(endo_scalars);
    return res;
}

g1::element pippenger(fr::field_t *scalars, g1::affine_element *points, size_t num_initial_points, size_t forced_bucket_width) 
{
    fr::field_t *endo_scalars = (fr::field_t *)aligned_alloc(32, sizeof(fr::field_t) * (num_initial_points));
    for (size_t i = 0; i < num_initial_points; ++i)
    {
...
g1::element pippenger_precomputed(fr::field_t *scalars, const std::vector<g1::affine_element*> &round_points, const size_t num_initial_points)
{
    fr::field_t *endo_scalars = (fr::field_t *)aligned_alloc(32, sizeof(fr::field_t) * (num_initial_points));
    for (size_t i = 0; i < num_initial_points; ++i)
    {
...
g1::element pippenger_internal_precomputed(fr::field_t *scalars, const std::vector<g1::affine_element*> &round_points, const size_t num_initial_points, fr::field_t *endo_scalars)
{
    multiplication_runtime_state state;
...
g1::element pippenger_internal(fr::field_t *scalars, g1::affine_element *points, size_t num_initial_points, fr::field_t *endo_scalars, size_t forced_bucket_width) 
{
    multiplication_runtime_state state;
    wnaf_runtime_state wnaf_state;
