...
template <typename base_field, typename Fq6Params> class field6 {
  public:
    ...
    constexpr field6 operator*(const field6& other) const
    {
        ...
        r.c0 = T0 + mul_by_non_residue(T5 - (T1 + T2));
        r.c1 = T4 - (T0 + T1) + mul_by_non_residue(T2);
        r.c2 = T3 + T1 - (T0 + T2);
    ...
    constexpr field6 sqr() const
    {
        ...
        r.c0 = mul_by_non_residue(S3) + S0;
        r.c1 = mul_by_non_residue(S4) + S1;
        r.c2 = S1 + S2 + S3 - S0 - S4;
    ...
    static constexpr field6 random_element(std::mt19937_64* engine = nullptr,
                                           std::uniform_int_distribution<uint64_t>* dest = nullptr)
    {
        field6 r;
        r.c0 = base_field::random_element(engine, dest);
        r.c1 = base_field::random_element(engine, dest);
        r.c2 = base_field::random_element(engine, dest);
        return r;
    }

    constexpr field6 to_montgomery_form() const
    {
        return { c0.to_montgomery_form(), c1.to_montgomery_form(), c2.to_montgomery_form() };
    }

    constexpr field6 from_montgomery_form() const
    {
        return { c0.from_montgomery_form(), c1.from_montgomery_form(), c2.from_montgomery_form() };
    }
