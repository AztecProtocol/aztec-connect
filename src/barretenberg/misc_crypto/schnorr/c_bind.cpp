...
WASM_EXPORT void compute_public_key(uint8_t const* private_key, uint8_t* public_key_buf)
{
    ...
    grumpkin::g1::affine_element pub_key = grumpkin::g1::group_exponentiation(grumpkin::g1::affine_one, priv_key);
    grumpkin::g1::serialize_to_buffer(pub_key, public_key_buf);
}
...
WASM_EXPORT bool verify_signature(uint8_t const* message,
                                  ...
                                  uint8_t const* sig_e)
{
    auto pubk = grumpkin::g1::serialize_from_buffer(const_cast<uint8_t*>(pub_key));
