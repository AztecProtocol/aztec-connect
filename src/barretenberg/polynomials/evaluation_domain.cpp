...
void compute_lookup_table_single(const fr::field_t& input_root,
                                 const size_t size,
                                 fr::field_t* const roots,
                                 std::vector<fr::field_t*>& round_roots)
{
...
evaluation_domain::evaluation_domain(const size_t domain_size, const size_t target_generator_size)
    ...
    , root(fr::field_t::get_root_of_unity(log2_size))
    , root_inverse(root.invert())
    , domain(fr::field_t{ size, 0, 0, 0 }.to_montgomery_form())
    , domain_inverse(domain.invert())
    , generator(fr::field_t::coset_generator(0))
    , generator_inverse(fr::field_t::coset_generator(0).invert())
    , roots(nullptr)
{
