...
void fft_inner_parallel_old(fr::field_t* coeffs,
                            const evaluation_domain& domain,
                            const fr::field_t&,
                            const fr::field_t** root_table)
{
    if (domain.size < 32)
    {
...
fr::field_t compute_kate_opening_coefficients(fr::field_t* coeffs, const fr::field_t& z, const size_t n)
{
    ...
    fr::__sub_with_coarse_reduction(coeffs[0], f, coeffs[0]);
    fr::__mul_without_reduction(coeffs[0], divisor, coeffs[0]);
    for (size_t i = 1; i < n; ++i)
    {
        fr::__sub_with_coarse_reduction(coeffs[i], coeffs[i - 1], coeffs[i]);
        fr::__mul_without_reduction(coeffs[i], divisor, coeffs[i]);
    }
