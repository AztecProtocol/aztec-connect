...
bool join_split(rollup_context& ctx, join_split_tx const& tx)
{
    ...
    stdlib::point owner_pub_key = { witness_t(&ctx.composer, tx.owner_pub_key.x),
                                    witness_t(&ctx.composer, tx.owner_pub_key.y) };
    stdlib::schnorr::signature_bits signature = stdlib::schnorr::convert_signature(&ctx.composer, tx.signature);
    stdlib::pedersen_note::public_note notes[4] = {
