...
protected:
  ...
  root_verifier_tests() : context(js_cd, account_cd, claim_cd) {}
  ...
  static void SetUpTestCase() {
    ...
    if (recreate) {
      tx_rollup_cd =
          rollup::get_circuit_data(1U, js_cd, account_cd, claim_cd, srs,
                                   FIXTURE_PATH, true, persist, persist);
      // create 1x1 circuit data; this will be the only shape accepted by the
      // root verifier circuit.
      root_rollup_cd = root_rollup::get_circuit_data(
          1U, tx_rollup_cd, srs, FIXTURE_PATH, true, persist, persist);
      root_verifier_cd = root_verifier::get_circuit_data(
          root_rollup_cd, srs, {root_rollup_cd.verification_key}, FIXTURE_PATH,
          true, persist, persist);
      // create 1x2 key to use later
      root_rollup_cd_bad = root_rollup::get_circuit_data(
          2U, tx_rollup_cd, srs, FIXTURE_PATH, true, persist, persist);
    } else {
      tx_rollup_cd = rollup::get_circuit_data(1U, js_cd, account_cd, claim_cd,
                                              srs, FIXTURE_PATH, false, false,
                                              true, false, true);
      root_rollup_cd = root_rollup::get_circuit_data(
          1U, tx_rollup_cd, srs, FIXTURE_PATH, false, false, true, false, true);
      root_verifier_cd = get_circuit_data(
          root_rollup_cd, srs, {root_rollup_cd.verification_key}, FIXTURE_PATH,
          false, false, true, false, true);
      // create 1x2 key to use later
      root_rollup_cd_bad = root_rollup::get_circuit_data(
          2U, tx_rollup_cd, srs, FIXTURE_PATH, false, false, true, false, true);
    }
  }

  root_verifier_tx create_root_verifier_tx() {
    auto root_rollup = fixtures::compute_or_load_fixture(
        TEST_PROOFS_PATH, "root_rollup", [&]() {
          auto js_proof =
              context.create_join_split_proof({}, {}, {100, 50}, 150);
          auto rollup_tx = rollup::create_rollup_tx(
              context.world_state, tx_rollup_cd.rollup_size, {js_proof});
          ...
          auto root_rollup_tx = root_rollup::create_root_rollup_tx(
              ...
              context.world_state.defi_tree.get_hash_path(0), {rollup_data});
