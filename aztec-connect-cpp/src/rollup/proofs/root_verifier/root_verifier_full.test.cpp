...
protected:
  ...
  static constexpr auto FIXTURE_PATH =
      "../src/rollup/proofs/root_verifier/fixtures";
  static constexpr auto TEST_PROOFS_PATH =
      "../src/rollup/proofs/root_verifier/fixtures/test_proofs";
  ...
  static void SetUpTestCase() {
    ...
    tx_rollup_cd =
        rollup::get_circuit_data(1, join_split_cd, account_cd, claim_cd, srs,
                                 FIXTURE_PATH, true, false, false);
    root_rollup_cd = root_rollup::get_circuit_data(
        1, tx_rollup_cd, srs, FIXTURE_PATH, true, false, false);
    root_verifier_cd =
        get_circuit_data(root_rollup_cd, srs, {root_rollup_cd.verification_key},
                         FIXTURE_PATH, true, false, false);

    MemoryStore store;
    MerkleTree<MemoryStore> data_tree(store, DATA_TREE_DEPTH, 0);
    js_proof = join_split::create_noop_join_split_proof(join_split_cd,
                                                        data_tree.root());
  }

  root_verifier_tx create_root_verifier_tx() {
    auto rollup_tx = rollup::create_rollup_tx(
        world_state, tx_rollup_cd.rollup_size, {js_proof});
    ...
    auto root_rollup_tx = root_rollup::create_root_rollup_tx(
        world_state, 0, world_state.defi_tree.root(),
        world_state.defi_tree.get_hash_path(0), {rollup_data});
...
HEAVY_TEST_F(root_verifier_full_tests,
             good_data_passes_and_detect_circuit_change) {
  ...
  if (!(circuit_gate_count::is_circuit_change_expected)) {
    ...
    EXPECT_EQ(from_buffer<uint256_t>(vk_hash_root_verifier),
              circuit_vk_hash::ROOT_VERIFIER)
        << "The verification key hash for the root verifier circuit is "
           "changed.";
    ...
    EXPECT_LE(number_of_gates_root_verifier,
              circuit_gate_next_power_of_two::ROOT_VERIFIER -
                  waffle::ComposerBase::NUM_RESERVED_GATES)
        << "You have exceeded the next power of two limit for the root "
           "verifier circuit.";
  } else {
    EXPECT_LE(number_of_gates_root_verifier,
              circuit_gate_next_power_of_two::ROOT_VERIFIER -
...
HEAVY_TEST_F(root_verifier_full_tests, bad_valid_point_failure) {
  ...
  /*
   * Check that the first recursive proof element occurring in tx.proof_data is
   * not the identity element of the curve (exceedingly unlikely). Then invert
   * this element and check that the proof does not verify. We do this inversion
   * 'by hand', inverting the y-coordinate, for simplicity.
   */
  ...
  // skip some public inputs fields and the x-coordinate of the first recursive
  // proof element.
