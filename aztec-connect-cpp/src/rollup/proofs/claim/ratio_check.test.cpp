...
TEST(ratio_check, product_check) {
  ...
  waffle::TurboComposer composer =
      waffle::TurboComposer("../barretenberg/cpp/srs_db/ignition");
  ...
  auto result = product_check(composer, left1, right1, left2, right2,
                              witness_ct(&composer, 0));
...
TEST(ratio_check, product_check_with_zeros) {
  ...
  auto result =
      product_check(composer, a1, b1, a2, b2, witness_ct(&composer, 0));
  result.assert_equal(true);
...
TEST(ratio_check, ratio_check) {
  ...
  uint256_t c;
  while (c == 0) {
    c = engine.get_random_uint256(); // it'll 'never' happen, but just in case
                                     // it's 0, try again.
  }
  c.data[3] = c.data[3] & 0x0fffffffffffffffULL; // 60-bits

  // Notice: if b > c, then (b/c) > 1, so in the equation below, a * (b / c) can
  // overflow 256-bits if `a` is sufficiently big. This check is done within the
  // circuit by checking total_in >= user_in.
  if (b > c) {
    std::swap(b, c);
  }
...
TEST(ratio_check, field_modulus_overflow_fails) {
  ...
  // // = 2^(-1)
  uint256_t d(0xA1F0FAC9F8000001ULL, 0x9419F4243CDCB848ULL,
