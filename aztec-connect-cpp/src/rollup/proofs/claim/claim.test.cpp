...
protected:
  static void SetUpTestCase() {
    srs = std::make_shared<waffle::FileReferenceStringFactory>(
  ...
  virtual void SetUp() {
    ...
    data_tree =
        std::make_unique<MerkleTree<MemoryStore>>(*store, DATA_TREE_DEPTH, 0);
    defi_tree =
        std::make_unique<MerkleTree<MemoryStore>>(*store, DEFI_TREE_DEPTH, 1);
    user = rollup::fixtures::create_user_context();
  }

  template <typename T, typename Tree>
  void append_note(T const &note, Tree &tree) {
    tree->update_element(tree->size(), note.commit());
  }

  claim_tx create_claim_tx(claim_note const &claim_note,
                           uint32_t claim_note_index, uint32_t defi_note_index,
    ...
    tx.output_value_a = ((uint512_t(claim_note.deposit_value) *
                          uint512_t(interaction_note.total_output_value_a)) /
                         uint512_t(interaction_note.total_input_value))
                            .lo;
    tx.output_value_b = ((uint512_t(claim_note.deposit_value) *
                          uint512_t(interaction_note.total_output_value_b)) /
                         uint512_t(interaction_note.total_input_value))
                            .lo;
    return tx;
  }
  ...
  const uint32_t empty_virtual_asset_id = uint32_t(1)
                                          << (MAX_NUM_ASSETS_BIT_LENGTH - 1);
};
...
TEST_F(claim_tests, test_claim_and_detect_circuit_change) {
  const claim_note note1 = {
      ...
      .fee = 0,
      .value_note_partial_commitment = create_partial_commitment(
          user.note_secret, user.owner.public_key, 0, 0),
  ...
  if (!(circuit_gate_count::is_circuit_change_expected)) {
    ...
    EXPECT_EQ(from_buffer<uint256_t>(vk_hash_claim), circuit_vk_hash::CLAIM)
        ...
        << from_buffer<uint256_t>(vk_hash_claim);
    ...
    EXPECT_LE(number_of_gates_claim,
              circuit_gate_next_power_of_two::CLAIM -
                  waffle::ComposerBase::NUM_RESERVED_GATES)
        << "You have exceeded the next power of two limit for the claim "
           "circuit.";
  } else {
    EXPECT_LE(number_of_gates_claim,
              circuit_gate_next_power_of_two::CLAIM -
...
TEST_F(claim_tests, test_theft_via_field_overflow_fails_1) {
  // Choose values to retain the ratio (deposit * total_output_value_a) ==
  // (output_value_a * total_input_value) deposit value: 1 total_input_value: 2
  // total_output_value_a: 1
  // output_value_a:
  // 10944121435919637611123202872628637544274182200208017171849102093287904247809;
  // // = 2^(-1)

  uint256_t o_v_a(0xA1F0FAC9F8000001ULL, 0x9419F4243CDCB848ULL,
                  0xDC2822DB40C0AC2EULL, 0x183227397098D014ULL); // 2^(-1)
...
TEST_F(claim_tests, test_theft_via_field_overflow_fails_2) {
  // Choose values to retain the ratio (deposit * total_output_value_a) ==
  // (output_value_a * total_input_value) deposit value: 1 total_input_value: 74
  // // chosen (by brute force) so that the inverse is under 252 bits.
  // total_output_value_a: 1
  // output_value_a:
  // 295787065835665881381708185746719933629031951356973437077002759278051466157
  // // 74^(-1)

  uint256_t o_v_a(0x507c2274294c1badULL, 0x11d7301ca7b2f039ULL,
                  0x21a0384b1d6cfdbcULL, 0x00a768d809f64ad0ULL); // 74^(-1)
  ...
  append_note(note2, defi_tree);
  claim_tx tx = create_claim_tx(note1, 0, 0, note2);
  tx.output_value_a = o_v_a; // choose the cheeky large output value, that flies
                             // under the 252-bit radar

  auto result = verify_logic(tx, cd);
  EXPECT_FALSE(result.logic_verified);
  EXPECT_EQ(result.err, "safe_uint_t range constraint failure: subtract: "
                        "output_value_a > total_output_value_a");
}

TEST_F(claim_tests, test_integer_division_works) {
  // Choose a total_output_value_a which is not divisible (in the integers) by
  // the user's share. E.g. deposit 3 / 9. Then if the total output is 10, 1/3 *
  // 10 = 3.333333, so should yield '3', rather than some giant field element
  // (3^(-1) * 10). Tests to ensure the circuit copes with residuals correctly.
...
TEST_F(claim_tests, test_outputs_larger_than_252_bits_fails) {
  uint256_t r(0x43E1F593F0000001ULL, 0x2833E84879B97091ULL,
              0xB85045B68181585DULL, 0x30644E72E131A029ULL); // field modulus
  ...
  EXPECT_EQ(result.err,
            "safe_uint_t range constraint failure: total_output_value_a");
}
...
TEST_F(claim_tests, test_theft_via_zero_equality_fails) {
  ...
  uint256_t MAX_252_BIT_VALUE(0xffffffffffffffffULL, 0xffffffffffffffffULL,
  ...
  EXPECT_EQ(
      result.err,
      "Not supported: zero deposit"); // This case was already caught by the
                                      // ratio_check function preventing a
                                      // zero-valued denominator of b2 =
...
TEST_F(claim_tests, test_deposit_greater_than_total_fails) {
  ...
  claim_tx tx = create_claim_tx(note1, 0, 0, note2);
  tx.output_value_a =
      100; // Match the malicious ratio of the deposit_value:total_input_value

  auto result = verify_logic(tx, cd);
  EXPECT_FALSE(result.logic_verified);
  EXPECT_EQ(result.err, "safe_uint_t range constraint failure: subtract: "
                        "deposit_value > total_input_value");
}
...
TEST_F(claim_tests, test_zero_total_output_value_fails) {
  ...
  EXPECT_FALSE(result.logic_verified);
  EXPECT_EQ(result.err,
            "safe_uint_t range constraint failure: subtract: output_value_a > "
            "total_output_value_a"); // The 'division by
                                     // zero' checks aren't
                                     // even reached,
                                     // because this one
                                     // gets triggered
...
TEST_F(claim_tests, test_defi_note_incorrect_index_fails) {
  ...
  claim_tx tx_fail = create_claim_tx(
      note1, 0, 25, defi_notes[25]); // interaction index taken from interaction
...
TEST_F(claim_tests, test_claim_for_virtual_note) {
  const bridge_call_data bridge_call_data = {
      .bridge_address_id = 0,
      .input_asset_id_a = 0,
      .input_asset_id_b = 0,
      .output_asset_id_a = 0,
  ...
  const defi_interaction::note note2 = {.bridge_call_data =
  ...
  EXPECT_TRUE(result.logic_verified);
  EXPECT_EQ(tx.get_output_notes()[0],
            result.public_inputs[InnerProofFields::NOTE_COMMITMENT1]);
  EXPECT_EQ(tx.get_output_notes()[1],
            result.public_inputs[InnerProofFields::NOTE_COMMITMENT2]);
}
...
TEST_F(claim_tests, test_second_input_in_use_means_asset_ids_equal_fails) {
  const bridge_call_data bridge_call_data = {
      ...
      .output_asset_id_a = 0,
      .output_asset_id_b = 0,
      .config = bridge_call_data::bit_config{.second_input_in_use = true, // <--
                                             .second_output_in_use = false},
      .aux_data = 0};
...
TEST_F(claim_tests,
       test_second_output_in_use_means_real_output_asset_ids_equal_fails) {
  const bridge_call_data bridge_call_data = {
      .bridge_address_id = 0,
      .input_asset_id_a = 0,
      .input_asset_id_b = 0,
...
TEST_F(claim_tests,
       test_second_output_in_use_and_virtual_output_asset_ids_equal) {
  const bridge_call_data bridge_call_data = {
      .bridge_address_id = 0,
      .input_asset_id_a = 0,
      .input_asset_id_b = 0,
...
TEST_F(claim_tests,
       test_first_bridge_output_virtual_but_invalid_placeholder_fails) {
  const bridge_call_data bridge_call_data = {
      .bridge_address_id = 0,
      .input_asset_id_a = 0,
      .input_asset_id_b = 0,
...
TEST_F(claim_tests,
       test_second_bridge_output_virtual_but_invalid_placeholder_fails) {
  const bridge_call_data bridge_call_data = {
      .bridge_address_id = 0,
      .input_asset_id_a = 0,
      .input_asset_id_b = 0,
...
TEST_F(claim_tests, test_claim_2_outputs_full_proof) {
  ...
  // Create some values for our circuit that are large enough to properly test
  // the ratio checks. The defi deposit value must be atmost 242 bits (since we
  // sum up defi deposits in rollup circuit).
  auto random_value = []() {
  ...
  // Check total_in >= user_in. Does not work otherwise because we get integer
  // overflow.
...
// For less verbose code, we set up some default test data here. Individual
// elements of the test_data can then be modified in each test.
class test_data {
private:
  const uint32_t empty_virtual_asset_id =
...
public:
  ...
  test_data(rollup::fixtures::user_context user,
    ...
    bid = {
        .bridge_address_id = 123,
        .input_asset_id_a =
            456 + (virtual_flags.in1 ? empty_virtual_asset_id : 0),
        .input_asset_id_b = (in_use.in2 ? 789 : 0) +
                            (virtual_flags.in2 ? empty_virtual_asset_id : 0),
        .output_asset_id_a = virtual_flags.out1 ? empty_virtual_asset_id : 111,
        .output_asset_id_b =
            in_use.out2 ? (virtual_flags.out2 ? empty_virtual_asset_id : 222)
                        : 0,
        .config =
            bridge_call_data::bit_config{.second_input_in_use = in_use.in2,
...
// Elements of bridge_call_data are implicitly range-constrained by the
// bit-shifting in bridge_call_data.hpp (since bits outside of the valid ranges
// are ignored)

// Can't create tests which attempt to exceed 32-bit range for values which are
// 'fed in' as uint32_t
...
TEST_F(claim_tests, test_total_input_value_out_of_range_fails) {
  ...
  EXPECT_FALSE(result.logic_verified);
  EXPECT_EQ(result.err,
            "safe_uint_t range constraint failure: total_input_value");
}
...
TEST_F(claim_tests, test_total_output_value_b_out_of_range_fails) {
  ...
  EXPECT_FALSE(result.logic_verified);
  EXPECT_EQ(result.err,
            "safe_uint_t range constraint failure: total_output_value_b");
}
