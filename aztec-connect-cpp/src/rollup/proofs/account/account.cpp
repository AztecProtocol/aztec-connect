...
field_ct
compute_account_alias_hash_nullifier(suint_ct const &account_alias_hash) {
  return pedersen::compress(
      std::vector<field_ct>{account_alias_hash.value},
      notes::GeneratorIndex::ACCOUNT_ALIAS_HASH_NULLIFIER);
}

field_ct
compute_account_public_key_nullifier(point_ct const &account_public_key) {
  return pedersen::compress(
      std::vector<field_ct>{account_public_key.x},
      notes::GeneratorIndex::ACCOUNT_PUBLIC_KEY_NULLIFIER);
}
void account_circuit(Composer &composer, account_tx const &tx) {
  // @dev This has to be a witness because we want to set it as a public input
  // (see set_public() later). However, we don't want provers to have freedom to
  // change this value.
  ...
  const auto account_public_key =
      stdlib::create_point_witness(composer, tx.account_public_key);
  const auto new_account_public_key =
      stdlib::create_point_witness(composer, tx.new_account_public_key);
  const auto spending_public_key_1 =
      stdlib::create_point_witness(composer, tx.new_signing_pub_key_1, false);
  const auto spending_public_key_2 =
      stdlib::create_point_witness(composer, tx.new_signing_pub_key_2, false);
  const auto alias_hash = suint_ct(witness_ct(&composer, tx.alias_hash),
  ...
  const auto account_note_index =
      suint_ct(witness_ct(&composer, tx.account_note_index), DATA_TREE_DEPTH,
               "account_note_index");
  const auto account_note_path =
      merkle_tree::create_witness_hash_path(composer, tx.account_note_path);
  const auto signing_pub_key =
  ...
  const auto output_note_1 =
      account_note(output_account_alias_hash.value, new_account_public_key,
                   spending_public_key_1);
  const auto output_note_2 =
      account_note(output_account_alias_hash.value, new_account_public_key,
                   spending_public_key_2);

  // @dev unlimited zero-valued nullifiers are permitted by the rollup circuit
  // (e.g. if create == 0).
  const auto nullifier_1 =
      compute_account_alias_hash_nullifier(alias_hash) * create;

  // If create or migrate, nullifier_2 = nullifier of the account_public_key
  // being registered.
  field_ct nullifier_2 = field_ct::conditional_assign(
      (create || migrate),
      compute_account_public_key_nullifier(new_account_public_key), 0);

  // If creating an account from scratch, sign against the account private key,
  // else sign with the spending key of the input note
  const point_ct signer =
      point_ct::conditional_assign(create, account_public_key, signing_pub_key);

  // Validate that account public key != account spending key for output notes
  new_account_public_key.assert_not_equal(
      spending_public_key_1, "account note 1: public key matches spending key");
  new_account_public_key.assert_not_equal(
      spending_public_key_2, "account note 2: public key matches spending key");

  // Validate that both create and migrate are not set!
  (field_ct(create) * field_ct(migrate))
      .assert_is_zero("cannot both create and migrate an account");
  ...
  {
    const auto account_note_data =
        account_note(alias_hash.value, account_public_key, signer);
    const auto account_note_exists = merkle_tree::check_membership(
        data_tree_root, account_note_path, account_note_data.commitment,
        account_note_index.value.decompose_into_bits(DATA_TREE_DEPTH));
    (!create).must_imply(account_note_exists,
  ...
  // Check account public key does not change unless migrating.
  {
    const auto account_key_change =
        account_public_key.x != new_account_public_key.x ||
        account_public_key.y != new_account_public_key.y;
    account_key_change.must_imply(
        migrate, "cannot change account keys unless migrating");
  }
...
void init_proving_key(
    std::shared_ptr<waffle::ReferenceStringFactory> const &crs_factory,
    bool mock) {
  if (proving_key) {
    return;
  }
  ...
  if (!mock) {
    ...
    rollup::proofs::mock::mock_circuit(mock_proof_composer,
                                       composer.get_public_inputs());
    proving_key = mock_proof_composer.compute_proving_key();
  }
}

void init_proving_key(std::shared_ptr<waffle::ProverReferenceString> const &crs,
                      waffle::proving_key_data &&pk_data) {
  release_key();
  proving_key = std::make_shared<waffle::proving_key>(std::move(pk_data), crs);
}

void release_key() { proving_key.reset(); }
...
void init_verification_key(
  ...
  if (!proving_key) {
    ...
    proving_key->reference_string =
        crs_factory->get_prover_crs(proving_key->n + 1);
  }
  verification_key = waffle::turbo_composer::compute_verification_key(
      proving_key, crs_factory->get_verifier_crs());
}

void init_verification_key(
    std::shared_ptr<waffle::VerifierMemReferenceString> const &crs,
    waffle::verification_key_data &&vk_data) {
  verification_key =
      std::make_shared<waffle::verification_key>(std::move(vk_data), crs);
}
...
bool verify_proof(waffle::plonk_proof const &proof) {
  UnrolledVerifier verifier(
      verification_key,
      Composer::create_unrolled_manifest(verification_key->num_public_inputs));

  std::unique_ptr<waffle::KateCommitmentScheme<waffle::unrolled_turbo_settings>>
      kate_commitment_scheme = std::make_unique<
          waffle::KateCommitmentScheme<waffle::unrolled_turbo_settings>>();
  verifier.commitment_scheme = std::move(kate_commitment_scheme);

  return verifier.verify_proof(proof);
}

std::shared_ptr<waffle::proving_key> get_proving_key() { return proving_key; }

std::shared_ptr<waffle::verification_key> get_verification_key() {
  return verification_key;
}

size_t get_number_of_gates() { return number_of_gates; }
