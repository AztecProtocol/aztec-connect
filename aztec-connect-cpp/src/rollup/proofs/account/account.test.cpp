...
protected:
  static void SetUpTestCase() {
    auto crs_factory = std::shared_ptr<waffle::ReferenceStringFactory>(
        new waffle::FileReferenceStringFactory(
  ...
  void preload_account_notes() {
    tree->update_element(
        tree->size(),
        create_account_leaf_data(alice.alias_hash, alice.owner.public_key,
                                 alice.signing_keys[0].public_key));
    tree->update_element(
        tree->size(),
        create_account_leaf_data(alice.alias_hash, alice.owner.public_key,
                                 alice.signing_keys[1].public_key));
  }
  ...
  uint256_t compute_account_alias_hash_nullifier(fr const &account_alias_hash) {
    ...
    auto result = crypto::pedersen::compress_native(
        hash_elements, notes::GeneratorIndex::ACCOUNT_ALIAS_HASH_NULLIFIER);
    return uint256_t(result);
  }

  uint256_t compute_account_public_key_nullifier(
      grumpkin::g1::affine_element const &account_public_key) {
  ...
  account_tx create_migrate_account_tx(
      const rollup::fixtures::user_context &user,
  ...
  account_tx create_add_signing_keys_account_tx(
      const rollup::fixtures::user_context &user,
      const rollup::fixtures::grumpkin_key_pair new_signing_keys[2]) {
    account_tx tx;
    tx.merkle_root = tree->root();
...
TEST_F(account_tests, test_migrate_account_with_account_key_fails) {
  ...
  // Set the signing key to equal the owner public key and sign with the public
  // key. The signature will be correct, but the circuit will look for an
  // account note with the "wrong" owner public key
...
TEST_F(account_tests, test_wrong_alias_hash_fails) {
  preload_account_notes();
  auto tx = create_add_signing_keys_account_tx(alice, bob.signing_keys);
  // The circuit will calculate an 'old' account note with the wrong alias, so
  // the membership check should fail.
  tx.alias_hash = rollup::fixtures::generate_alias_hash(
      "penguin"); // it's actually "pebble"
...
TEST_F(account_tests,
       test_create_account_when_account_exists_creates_nullifier_collision) {
  preload_account_notes();
  auto tx = create_new_account_tx(alice);
  auto prover = new_account_prover(tx, false);
  auto proof = prover.construct_proof();
  ...
  auto note1_commitment = account_note{tx.alias_hash, tx.account_public_key,
                                       tx.new_signing_pub_key_1}
                              .commit();
  auto note2_commitment = account_note{tx.alias_hash, tx.account_public_key,
                                       tx.new_signing_pub_key_2}
                              .commit();
  ...
  EXPECT_EQ(data.nullifier2,
            compute_account_public_key_nullifier(
                alice.owner.public_key)); // public key of new acct
  ...
  EXPECT_EQ(data.nullifier1,
            compute_account_alias_hash_nullifier(alice.alias_hash));
}

TEST_F(account_tests,
       test_create_account_full_proof_and_detect_circuit_change) {
  auto tx = create_new_account_tx(alice);
  auto prover = new_account_prover(tx, false);
  auto proof = prover.construct_proof();
  auto data = inner_proof_data(proof.proof_data);
  ...
  EXPECT_EQ(data.note_commitment2, note2_commitment);
  EXPECT_EQ(data.nullifier1,
            compute_account_alias_hash_nullifier(tx.alias_hash));
  EXPECT_EQ(data.nullifier2,
            compute_account_public_key_nullifier(
                alice.owner.public_key)); // public key of new acct
  EXPECT_EQ(data.public_value, 0);
  ...
  if (!(circuit_gate_count::is_circuit_change_expected)) {
    ...
    EXPECT_EQ(from_buffer<uint256_t>(vk_hash_acc), circuit_vk_hash::ACCOUNT)
        ...
        << from_buffer<uint256_t>(vk_hash_acc);
    // For the next power of two limit, we need to consider that we reserve four
    // gates for adding randomness/zero-knowledge
    EXPECT_LE(number_of_gates_acc, circuit_gate_next_power_of_two::ACCOUNT -
                                       waffle::ComposerBase::NUM_RESERVED_GATES)
        << "You have exceeded the next power of two limit for the account "
           "circuit.";
  } else {
    EXPECT_LE(number_of_gates_acc, circuit_gate_next_power_of_two::ACCOUNT -
                                       waffle::ComposerBase::NUM_RESERVED_GATES)
...
TEST_F(account_tests, test_migrate_account_full_proof) {
  ...
  auto note1_commitment =
      account_note{.alias_hash = tx.alias_hash,
  ...
  EXPECT_EQ(data.nullifier2,
            compute_account_public_key_nullifier(new_account_key.public_key));
  EXPECT_EQ(data.public_value, 0);
  EXPECT_EQ(data.public_owner, fr(0));
  EXPECT_EQ(data.asset_id, uint256_t(0));
  EXPECT_EQ(data.merkle_root, tx.merkle_root);
