...
TEST_F(join_split_js_parity_tests, test_full_proof) {
  auto private_key = from_buffer<grumpkin::fr>(std::vector<uint8_t>{
      0x0b, 0x9b, 0x3a, 0xde, 0xe6, 0xb3, 0xd8, 0x1b, 0x28, 0xa0, 0x88,
      0x6b, 0x2a, 0x84, 0x15, 0xc7, 0xda, 0x31, 0x29, 0x1a, 0x5e, 0x96,
      0xbb, 0x7a, 0x56, 0x63, 0x9e, 0x17, 0x7d, 0x30, 0x1b, 0xeb});
  auto public_key = grumpkin::g1::one * private_key;
  auto note_secret = from_buffer<barretenberg::fr>(std::vector<uint8_t>{
      0x00, 0x00, 0x00, 0x00, 0x11, 0x11, 0x11, 0x11, 0x00, 0x00, 0x00,
      0x00, 0x11, 0x11, 0x11, 0x11, 0x00, 0x00, 0x00, 0x00, 0x11, 0x11,
      0x11, 0x11, 0x00, 0x00, 0x00, 0x00, 0x11, 0x11, 0x11, 0x11});
  ...
  auto input_note1_nullifier =
      compute_nullifier(input_note1.commit(), private_key, true);
  auto input_note2_nullifier =
      compute_nullifier(input_note2.commit(), private_key, true);
  value::value_note output_note1 = {
      80, 0, 0, public_key, note_secret, 0, input_note1_nullifier};
  value::value_note output_note2 = {
      50, 0, 0, public_key, note_secret, 0, input_note2_nullifier};
  ...
  // for (auto& e : stats) {
  //     info(e.first, ": reads: ", e.second.second, " writes: ",
  //     e.second.first);
  // }
}
