...
auto create_account_leaf_data(fr const &account_alias_hash,
  ...
  return notes::native::account::account_note{account_alias_hash, owner_key,
                                              signing_key}
      .commit();
}
...
protected:
  ...
  static void SetUpTestCase() {
    ...
    auto crs_factory = std::make_unique<waffle::FileReferenceStringFactory>(
        "../barretenberg/cpp/srs_db/ignition");
  ...
  virtual void SetUp() {
    ...
    const uint32_t virtual_asset_id =
        virtual_asset_id_flag + defi_interaction_nonce;
    ...
    // Similar to value_notes[0], but a different value of 90, to match
    // defi_deposit_value in tests.
    value_notes[8].value = 90;
    value_notes[8].creator_pubkey = input_user.owner.public_key.x;

    // Similar previous virtual notes, but a different value of 90, to match
    // defi_deposit_value in tests.
  ...
  void preload_account_notes() {
    tree->update_element(tree->size(),
                         create_account_leaf_data(
                             input_user.alias_hash, input_user.owner.public_key,
   ...
   */
  join_split_tx
  create_join_split_tx(std::array<uint32_t, 2> const &input_indices,
    ...
    auto input_nullifier1 = compute_nullifier(
        input_notes[0].commit(), input_user.owner.private_key, true);
    auto input_nullifier2 = compute_nullifier(
        input_notes[1].commit(), input_user.owner.private_key, true);
    value::value_note output_note1 = {.value = input_notes[0].value +
                                               input_notes[1].value,
    ...
    tx.input_path = {tree->get_hash_path(input_indices[0]),
                     tree->get_hash_path(input_indices[1])};
    ...
    tx.alias_hash = !account_required
                        ? rollup::fixtures::generate_alias_hash("penguin")
                        : input_user.alias_hash;
   ...
   */
  join_split_tx
  simple_setup(std::array<uint32_t, 2> const &input_indices = {0, 1},
               uint32_t account_note_index = 0, bool account_required = false,
    ...
    return create_join_split_tx(
        input_indices,
        {value_notes[input_indices[0]], value_notes[input_indices[1]]},
        tx_output_user ? *tx_output_user : output_user, account_note_index,
        account_required);
     ...
     * output_note_1: value = 150, asset_id = 1, account_required = 0, same
     * owner as inputs output_note_2: value = 0,   asset_id = 1,
     * account_required = 0, same owner as inputs
     */
  }

  join_split_tx
  same_owner_setup(std::array<uint32_t, 2> const &input_indices = {0, 1},
                   uint32_t account_note_index = 0,
                   bool account_required = false) {
    return simple_setup(input_indices, account_note_index, account_required,
                        &input_user);
  }
  ...
  join_split_tx zero_input_setup() {
    value::value_note input_note1 = {0,
                                     0,
                                     0,
                                     input_user.owner.public_key,
                                     input_user.note_secret,
                                     0,
                                     fr::random_element()};
    value::value_note input_note2 = {0,
                                     0,
                                     0,
                                     input_user.owner.public_key,
                                     input_user.note_secret,
                                     ...
                                     fr::random_element()};
    auto input_nullifier1 = compute_nullifier(
        input_note1.commit(), input_user.owner.private_key, false);
    auto input_nullifier2 = compute_nullifier(
        input_note2.commit(), input_user.owner.private_key, false);
    value::value_note output_note1 = {0,
                                      0,
                                      0,
                                      input_user.owner.public_key,
                                      input_user.note_secret,
                                      0,
                                      input_nullifier1};
    value::value_note output_note2 = {0,
                                      0,
                                      0,
                                      input_user.owner.public_key,
                                      input_user.note_secret,
  ...
  waffle::plonk_proof sign_and_create_proof(join_split_tx &tx,
                                            key_pair const &signing_key) {
  ...
  verify_result sign_and_verify_logic(join_split_tx &tx,
                                      key_pair const &signing_key) {
    tx.signature = sign_join_split_tx(tx, signing_key);
  ...
  bridge_call_data empty_bridge_call_data = {
      .bridge_address_id = 0,
      .input_asset_id_a = 0,
      .input_asset_id_b = 0,
      .output_asset_id_a = 0,
  ...
  const uint32_t virtual_asset_id_flag =
      (uint32_t(1) << (MAX_NUM_ASSETS_BIT_LENGTH - 1));
 ...
 * - "input asset ids must be different for the second bridge input to be
 * in-use"
 *     - config.second_input_in_use but equality: input_asset_id_a ==
 * input_asset_id_b (bridge inputs)
 * - "real output asset ids must be different for the second bridge output to be
 * in-use"
 *     - config.second_output_in_use and both real but equality:
 * output_asset_id_a == output_asset_id_b (bridge outputs)
 * - "output_asset_id_a detected as virtual, but has incorrect placeholder
 * value"
 *     - first output virtual, but output_asset_id_a != 2**29
 * - "output_asset_id_b detected as virtual, but has incorrect placeholder
 * value"
 ...
 * but defi_deposit_value != input_note_2_value
 * - "Expected bridge_call_data_local.input_asset_id_a == input_note_1.asset_id
 * for a defi-deposit"
 *     - is_defi_deposit, but bridge_call_data_local.input_asset_id_a !=
 * input_note_1.asset_id
 * - "Expected input_note_2_in_use, given
 * bridge_call_data_local.config.second_input_in_use"
 *     - bridge_call_data_local.config.second_input_in_use, but
 * input_note_2_in_use = false
 * - "Expected bridge_call_data_local.config.second_input_in_use, given
 * input_note_2_in_use && different_input_asset_ids"
 *     - input_note_2_in_use && different_input_asset_ids, but
 * bridge_call_data_local.config.second_input_in_use = false,
 * - "Expected bridge_call_data_local.input_asset_id_b == input_note_2.asset_id,
 * given bridge_call_data_local.config.second_input_in_use"
 *     - bridge_call_data_local.config.second_input_in_use, but
 * bridge_call_data_local.input_asset_id_b != input_note_2.asset_id
 *
 ...
 *     - defi deposit, input_note_2 in use, different input note asset_ids,
 * input_note_1_value < input_note_2_value
 *
 * ACCOUNT
 *
 ...
 *     - input an incorrect inputs.account_private_key, relative to the
 * input_note_1.owner
 * - "account_required incorrect"
 *     - set inputs.account_required != input_note_1.account_required
 * - "output note 1 creator_pubkey mismatch"
 *     - set the output_note_1.creator_pubkey to be different from
 * input_note_1.owner (and nonzero)
 * - "output note 2 creator_pubkey mismatch"
 *     - set the output_note_2.creator_pubkey to be different from
 * input_note_1.owner (and nonzero)
...
TEST_F(join_split_tests, test_invalid_num_input_notes_fails) {
  ...
  tx.output_note[1].input_nullifier = compute_nullifier(
      tx.input_note[1].commit(), input_user.owner.private_key, false);
...
TEST_F(join_split_tests, test_0_input_notes_and_detect_circuit_change) {
  ...
  // If the below assertions fail, consider changing the variable
  // is_circuit_change_expected to 1 in rollup/constants.hpp and see if atleast
  // the next power of two limit is not exceeded. Please change the constant
  // values accordingly and set is_circuit_change_expected to 0 in
  // rollup/constants.hpp before merging.
  if (!(circuit_gate_count::is_circuit_change_expected)) {
    ...
    // For the next power of two limit, we need to consider that we reserve four
    // gates for adding randomness/zero-knowledge
    EXPECT_LE(number_of_gates_js, circuit_gate_next_power_of_two::JOIN_SPLIT -
                                      waffle::ComposerBase::NUM_RESERVED_GATES)
        << "You have exceeded the next power of two limit for the join_split "
           "circuit.";
  } else {
    EXPECT_LE(number_of_gates_js, circuit_gate_next_power_of_two::JOIN_SPLIT -
                                      waffle::ComposerBase::NUM_RESERVED_GATES)
...
TEST_F(join_split_tests,
  ...
  tx.output_note[1] = tx.output_note[0]; // <-- attempt to maliciously create
                                         // duplicate output notes.
  tx.input_note[1] = tx.input_note[0];   // <-- for output notes to be equal,
                                         // input_nullifiers must be equal.
...
TEST_F(join_split_tests,
  ...
  tx.input_note[1].secret +=
      1; // <-- to avoid 'joining same note', modify input_note[1], but then hit
         // the error that requirement for different input_nullifiers will force
         // the output_notes to be different.
...
TEST_F(join_split_tests, test_1_input_note_with_num_input_notes_as_0) {
  join_split_tx tx = simple_setup();
  tx.num_input_notes = 0; // try to trick the circuit into creating a second
                          // option for a nullifier where is_real = 0.
  // tx.input_note[0] is nonzero - we're going to try to spend it with a cheeky
  // nullifier
  tx.input_note[1].value = 0;
  tx.output_note[0].value = tx.input_note[0].value;
  // create a cheeky nullifier for tx.input_note[0] where is_real = false
  tx.output_note[0].input_nullifier = compute_nullifier(
      tx.input_note[0].commit(), input_user.owner.private_key, false);
  tx.output_note[1].input_nullifier = compute_nullifier(
      tx.input_note[1].commit(), input_user.owner.private_key, false);
...
class test_valid_allow_chain_permutations
...
INSTANTIATE_TEST_SUITE_P(join_split_tests, test_valid_allow_chain_permutations,
                         ::testing::Values(0, 1, 2, 3));

TEST_F(join_split_tests, test_allow_chain_out_of_range_fails) {
  join_split_tx tx = simple_setup();
  tx.backward_link = fr::random_element(); // choose a value unrelated to the
                                           // inputs being spent
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
...
class test_allow_chain_to_other_users_fail
    : public join_split_tests,
      public ::testing::WithParamInterface<uint32_t> {};
TEST_P(test_allow_chain_to_other_users_fail, ) {
  join_split_tx tx = simple_setup();
  tx.allow_chain = GetParam();
  tx.output_note[tx.allow_chain - 1].owner =
      grumpkin::g1::element::random_element(); // i.e. not owned by self.
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "inter-user chaining disallowed");
}
INSTANTIATE_TEST_SUITE_P(join_split_tests, test_allow_chain_to_other_users_fail,
                         ::testing::Values(1, 2));
...
class test_propagated_notes_skip_membership_check
    : public join_split_tests,
...
TEST_P(test_propagated_notes_skip_membership_check, ) {
  ...
  tx.input_path[indicator - 1] =
      tree->get_hash_path(99); // select a clearly incorrect path for the input
                               // note being propagated.
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_TRUE(result.valid);
}
...
TEST_F(join_split_tests, test_propagated_input_note1_no_double_spend) {
  ...
  tx.output_note[0] = {tx.input_note[0].value,
                       tx.input_note[0].asset_id,
                       tx.input_note[0].account_required,
                       tx.input_note[0].owner,
                       tx.input_note[0].secret,
                       tx.input_note[0].creator_pubkey,
                       tx.output_note[0].input_nullifier};
  tx.output_note[1] = {tx.input_note[0].value,
                       tx.input_note[0].asset_id,
                       tx.input_note[0].account_required,
                       tx.input_note[0].owner,
                       tx.input_note[0].secret + 1,
                       tx.input_note[0].creator_pubkey,
                       tx.output_note[0].input_nullifier};
...
TEST_F(join_split_tests, test_overflow_tx_fee_fails) {
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "safe_uint_t range constraint failure: subtract: "
...
TEST_F(join_split_tests, test_different_input_note_owners_fails) {
  ...
  tx.input_note[0].owner =
      grumpkin::g1::affine_element::hash_to_curve(1).second;
...
//     EXPECT_FALSE(result.valid);
//     EXPECT_EQ(result.err, "input scalar to fixed_base_scalar_mul_internal
//     cannot be 0");
...
TEST_F(join_split_tests, test_tainted_output_owner_fails) {
  ...
  uint8_t public_owner[32] = {0x01, 0xaa, 0x42, 0xd4, 0x72, 0x88, 0x8e, 0xae,
                              0xa5, 0x56, 0x39, 0x46, 0xeb, 0x5c, 0xf5, 0x6c,
                              0x81, 0x6,  0x4d, 0x80, 0xc6, 0xf5, 0xa5, 0x38,
                              0xcc, 0x87, 0xae, 0x54, 0xae, 0xdb, 0x75, 0xd9};
...
TEST_F(join_split_tests, test_merge_with_same_owners_same_amounts_sig_bypass) {
  ...
  // Since our recipient equals our sender, amounts are the same, and it is a
  // send, this passes See https://hackmd.io/@aztec-network/H10r1JqOo
  join_split_tx tx = same_owner_setup();
  tx.proof_id = ProofIds::SEND;
  // sign with the wrong signer
  // since this is a merge with same owners and amount, this will still pass
...
TEST_F(join_split_tests, test_wrong_note_hash_path_fails) {
  join_split_tx tx = simple_setup();
  auto gibberish_path = fr_hash_path(
      32, std::make_pair(fr::random_element(), fr::random_element()));
...
TEST_F(join_split_tests, test_defi_deposit_one_virtual_input) {
  ...
  tx.output_note[1].input_nullifier =
      compute_nullifier(tx.input_note[1].commit(), input_user.owner.private_key,
                        false); // input note 2 is a dummy note
...
TEST_F(
   ...
   * tx represents:
   *   - 100 in (virtual)
   *   - 110 in (virtual, different asset_id = virtual_asset_id + 1) <-- fails,
   * since input note 2 value should equal defi deposit value.
   *   - 90 deposited via bridge input 1
   *   - 90 deposited via bridge input 2
   *   - 10 paid as fee (in1's asset_id)
   */
...
TEST_F(
   ...
   * tx represents:
   *   - 100 in1 (real)
   *   - 110 in2 (real, different asset_id = asset_id + 1) <-- fails, since
   * input note 2 value should equal defi deposit value.
   *   - 90 deposited via bridge input 1
   *   - 90 deposited via bridge input 2
   *   - 10 paid as fee (in1's asset_id)
   */
...
TEST_F(
   ...
   * tx represents:
   *   - 100 in1 (real)
   *   - 90 in2 (real, different asset_id = asset_id + 1) <-- fails, since input
   * note 2 value should equal defi deposit value.
   *   - 95 deposited via bridge input 1
   *   - 95 deposited via bridge input 2 <-- not possible, given input note 2
   * value
...
TEST_F(join_split_tests, test_defi_allow_chain_1_fails) {
   ...
   *   - 100 out2 (real, same asset_id)
   *   - 0 paid as fee
   *   - trying to chain off output_note_1, which is not allowed for defi
   * deposits.
   */
...
TEST_F(
  ...
  bridge_call_data.config.second_input_in_use =
      false; // <-- to cause the contradiction
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
...
TEST_F(
  ...
  bridge_call_data.config.second_output_in_use =
      false; // <-- to cause the contradiction
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
...
TEST_F(
  ...
  bridge_call_data bridge_call_data = empty_bridge_call_data;
  bridge_call_data.input_asset_id_a = tx.input_note[0].asset_id;
  bridge_call_data.input_asset_id_b =
      tx.input_note[1].asset_id; // <-- same asset_id
  bridge_call_data.config.second_input_in_use = true;
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
   ...
   *   - 50 deposited via bridge input 2 <-- not allowed, since both inputs have
   * the same asset_id (so they should be combined into a single deposit of 100
   * via bridge input 1)
   *   - 90 out2
   *   - 10 paid as fee (in1's asset_id)
   */
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "input asset ids must be different for the second "
                        "bridge input to be in-use");
}
...
TEST_F(
  ...
  bridge_call_data.output_asset_id_b =
      virtual_asset_id_flag; // <-- same, but that's ok, since they're virtual
                             // asset_id placeholders.
   ...
   *   - 0 in2 (not in use)
   *   - 90 deposited via bridge input 1
   *   - expectation of two _virtual_ bridge outputs, so they have the same
   * asset_id (placeholder values of 2 ** 29).
   *   - 10 paid as fee (in1's asset_id)
   */
...
TEST_F(
  ...
  bridge_call_data.output_asset_id_b =
      asset_id; // <-- same - not ok, since they're both real output asset_ids
  bridge_call_data.config.second_output_in_use = true;
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
   ...
   *   - 0 in2 (not in use)
   *   - 90 deposited via bridge input 1
   *   - expectation of two bridge outputs, but they have the same real asset_id
   * (and so they should be combined into bridge output 1).
   *   - 10 paid as fee (in1's asset_id)
   */

  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "real output asset ids must be different for the "
                        "second bridge output to be in-use");
}
...
TEST_F(
  ...
  bridge_call_data.output_asset_id_a =
      ...
      1; // should just be virtual_asset_id_flag (=2**29)
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
   ...
   *   - 0 in2 (not in use)
   *   - 90 deposited via bridge input 1
   *   - expectation of one _virtual_ bridge output, but it has an invlaid
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "output_asset_id_a detected as virtual, but has "
...
TEST_F(
   ...
   *   - 0 in2 (not in use)
   *   - 90 deposited via bridge input 1
   *   - expectation of two _virtual_ bridge outputs, but the 2nd has an invlaid
   * placeholder asset_id.
   *   - 10 paid as fee (in1's asset_id)
   */
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "output_asset_id_b detected as virtual, but has "
                        "incorrect placeholder value");
}
...
TEST_F(join_split_tests,
  ...
  bridge_call_data bridge_call_data = empty_bridge_call_data;
  bridge_call_data.input_asset_id_a =
      tx.input_note[0].asset_id + 1; // wrong asset_id vs input notes
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "Expected bridge_call_data_local.input_asset_id_a == "
                        "input_note_1.asset_id for a defi-deposit");
}
...
TEST_F(
  ...
  bridge_call_data.input_asset_id_a = tx.input_note[0].asset_id;
  bridge_call_data.config.second_input_in_use =
      true; // <-- causes error, since there's no input note 2 that can be fed
            // into the second bridge input.
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "Expected input_note_2_in_use, given "
                        "bridge_call_data_local.config.second_input_in_use");
}
...
TEST_F(join_split_tests,
   ...
   *   - 90 in2 (virtual)
   *   - 90 deposited via bridge input 1 (real)
   *   - 90 SHOULD BE deposited via bridge input 2 (virtual), but
   * bridge_call_data is incorrect.
   *   - 10 paid as fee (in1's asset_id)
   */
...
TEST_F(join_split_tests,
  ...
  bridge_call_data.input_asset_id_a = tx.input_note[0].asset_id;
  bridge_call_data.input_asset_id_b =
      tx.input_note[1].asset_id +
      1; // <-- wrong asset_id, vs input note 2's asset_id
  bridge_call_data.config.second_input_in_use = true;
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  EXPECT_EQ(result.err, "Expected bridge_call_data_local.input_asset_id_b == "
                        "input_note_2.asset_id, given "
                        "bridge_call_data_local.config.second_input_in_use");
}
...
TEST_F(join_split_tests, test_repayment_logic) {
  ...
  tx.partial_claim_note.deposit_value = 90;
  tx.partial_claim_note.input_nullifier = tx.output_note[0].input_nullifier;
  tx.output_note[1].value =
      10; // <-- repaying some value back to the defi-depositor

  bridge_call_data bridge_call_data = empty_bridge_call_data;
  bridge_call_data.input_asset_id_a = tx.input_note[0].asset_id;
  bridge_call_data.input_asset_id_b =
      virtual_asset_id_flag + defi_interaction_nonce; // virtual_asset_id
  bridge_call_data.config.second_input_in_use = true;
  tx.partial_claim_note.bridge_call_data = bridge_call_data.to_uint256_t();
...
TEST_F(join_split_tests, test_virtual_note_repay_different_asset_id_fail) {
  ...
  tx.partial_claim_note.deposit_value = 90;
  tx.partial_claim_note.input_nullifier = tx.output_note[0].input_nullifier;
  tx.output_note[1].asset_id =
      3; // <-- different from any of the input notes' asset_ids
...
TEST_F(join_split_tests, test_real_input_value_lt_virtual_input_value_fails) {
   ...
   *   - 50 in1 (real)
   *   - 90 in2 (virtual)
   *   - 90 deposited via bridge input 1 (shouldn't be possible, given low in1
   * value)
   *   - 90 deposited via bridge input 2
   *   - 10 out2 repayment back to depositor (INVALID asset_id)
   */
  ...
  auto result = sign_and_verify_logic(tx, input_user.owner);
  EXPECT_FALSE(result.valid);
  // In this context, failure of this subtraction (due to underflow) implies
  // input note 1's value was less than input note 2's value (and hence the
  // defi_deposit_value) (which is not allowed).
  EXPECT_EQ(result.err, "safe_uint_t range constraint failure: subtract: "
                        "total_in_value < total_out_value");
}
...
TEST_F(join_split_tests, test_incorrect_output_note_creator_pubkey_x) {
  {
    ...
    tx.output_note[0].creator_pubkey =
        rollup::fixtures::create_key_pair(nullptr)
            .public_key.x; // setting creator to be different from sender (the
                           // owner of the input notes).
    auto result = sign_and_verify_logic(tx, input_user.owner);
    EXPECT_FALSE(result.valid);
...
TEST_F(join_split_tests, test_deposit_full_proof) {
  ...
  uint256_t nullifier1 = compute_nullifier(input_note1_commitment,
                                           input_user.owner.private_key, false);
  uint256_t nullifier2 = compute_nullifier(input_note2_commitment,
                                           input_user.owner.private_key, false);
...
TEST_F(join_split_tests, test_withdraw_full_proof) {
  ...
  auto input_note2_commitment = tx.input_note[1].commit();
  uint256_t nullifier1 = compute_nullifier(input_note1_commitment,
                                           input_user.owner.private_key, true);
  uint256_t nullifier2 = compute_nullifier(input_note2_commitment,
                                           input_user.owner.private_key, true);
  auto output_note1_commitment = tx.output_note[0].commit();
  auto output_note2_commitment = tx.output_note[1].commit();
...
TEST_F(join_split_tests, test_defi_deposit_full_proof) {
  ...
  auto partial_value_commitment = value::create_partial_commitment(
      tx.partial_claim_note.note_secret, tx.input_note[0].owner,
      tx.input_note[0].account_required, 0);
  claim::claim_note claim_note = {tx.partial_claim_note.deposit_value,
                                  tx.partial_claim_note.bridge_call_data,
                                  0,
                                  0,
                                  partial_value_commitment,
  ...
  EXPECT_EQ(proof_data.bridge_call_data,
            tx.partial_claim_note.bridge_call_data);
