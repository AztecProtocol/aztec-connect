...
protected:
  static constexpr auto CRS_PATH = "../barretenberg/cpp/srs_db/ignition";
  static constexpr auto FIXTURE_PATH =
      "../src/rollup/proofs/root_rollup/fixtures";
  static constexpr auto TEST_PROOFS_PATH =
      "../src/rollup/proofs/root_rollup/fixtures/test_proofs";
  ...
  root_rollup_tests()
      : context(js_cd, account_cd, claim_cd), js_proofs(get_js_proofs(5)) {}
  ...
  static void SetUpTestCase() {
    ...
    if (recreate) {
      // If no fixtures dir, recreate all proving keys, verification keys,
      // padding proofs etc.
      tx_rollup_cd = rollup::get_circuit_data(
          INNER_ROLLUP_TXS, js_cd, account_cd, claim_cd, srs, FIXTURE_PATH,
          true, persist, persist);
    } else {
      // Otherwise we should only need the inner proofs verification key for
      // logic tests.
      tx_rollup_cd = rollup::get_circuit_data(
          INNER_ROLLUP_TXS, js_cd, account_cd, claim_cd, srs, FIXTURE_PATH,
    ...
    root_rollup_cd =
        get_circuit_data(ROLLUPS_PER_ROLLUP, tx_rollup_cd, srs, FIXTURE_PATH,
                         false, false, false, false, false);
  }

  root_rollup_tx create_root_rollup_tx(
      std::string const &test_name, RollupStructure const &rollup_structure,
      ...
      std::vector<native::defi_interaction::note> const &interaction_notes =
          {}) {
    uint32_t rollup_id =
        static_cast<uint32_t>(context.world_state.root_tree.size() - 1);
    auto old_defi_root = context.world_state.defi_tree.root();
    auto old_defi_path = context.world_state.defi_tree.get_hash_path(
        rollup_id * NUM_INTERACTION_RESULTS_PER_BLOCK);
    context.world_state.add_defi_notes(
        interaction_notes, rollup_id * NUM_INTERACTION_RESULTS_PER_BLOCK);
    ...
    for (size_t i = 0; i < rollup_structure.size(); ++i) {
      ...
      auto rollup = rollup::create_rollup_tx(
          context.world_state, INNER_ROLLUP_TXS, tx_proofs,
          bridge_call_datas[i], asset_ids[i]);
      auto fixture_name =
          format(test_name, "_rollup", rollup_id, "_inner", inner_data.size());
    ...
    return root_rollup::create_root_rollup_tx(
        context.world_state, rollup_id, old_defi_root, old_defi_path,
        inner_data, bridge_call_datas_union, asset_ids_union,
        interaction_notes);
  }

  std::vector<uint8_t> compute_or_load_rollup(std::string const &name,
                                              rollup::rollup_tx &rollup) {
    return fixtures::compute_or_load_fixture(TEST_PROOFS_PATH, name, [&] {
      // We need to ensure we have a proving key to build the inner proof
      // fixtures.
      if (!tx_rollup_cd.proving_key) {
        tx_rollup_cd = rollup::get_circuit_data(INNER_ROLLUP_TXS, js_cd,
                                                account_cd, claim_cd, srs,
                                                FIXTURE_PATH, true, true, true);
  ...
  std::vector<std::vector<uint8_t>> get_js_proofs(uint32_t n) {
    ...
    for (uint32_t i = 0; i < n; ++i) {
      auto js_proof = fixtures::compute_or_load_fixture(
          TEST_PROOFS_PATH, format("js", i), [&] {
  ...
  root_rollup_tx create_full_logic_root_rollup_tx() {
    ...
    auto js_proof1 = context.create_join_split_proof({0, 1}, {100, 50},
                                                     {70, 80 - 7}); // fee = 7
    auto js_proof2 = context.create_join_split_proof(
        {2, 3}, {100, 50}, {20, 130 - 15}, 0, 0, 0, aid1); // fee = 15
    auto js_proof3 = context.create_join_split_proof(
        {4, 5}, {100, 50}, {10, 140 - 9}, 0, 0, 0, aid2); // fee = 9
    auto defi_proof1 = context.create_defi_proof({6, 7}, {100, 50}, {30, 102},
                                                 bid2, aid2); // fee = 18
    auto defi_proof2 = context.create_defi_proof({8, 9}, {100, 50}, {50, 80},
                                                 bid2, aid2); // fee = 20
    auto defi_proof3 = context.create_defi_proof({10, 11}, {100, 50}, {20, 111},
                                                 bid3, aid3); // fee = 19

    // Add some defi interaction notes.
    std::vector<notes::native::defi_interaction::note> interaction_notes = {
        {1, 0, 3, 4, 5, false}, {2, 1, 4, 5, 6, true}};
    return create_root_rollup_tx("root_full_logic",
                                 {{js_proof1, js_proof2},
                                  {defi_proof1, defi_proof3},
                                  {js_proof3, defi_proof2}},
                                 {{}, {bid2, bid3}, {bid2}}, {bid2, bid3},
                                 {{0, aid1}, {aid2, aid3}, {aid2}},
                                 {0, aid1, aid3, aid2}, interaction_notes);
  }
...
/*
 * Due the the length of time it takes to produce inner proofs, they're saved in
 * fixtures. If they need to be recomputed due to a circuit change or otherwise,
 * delete files in ./fixtures/test_proofs. (You might need to delete all of the
 * fixtures). The fixtures' names are named so as to reduce unnecessary
 * (re)computation between tests. i.e. If a rollup has a structure shorter than
 * its name suggests, it's because it can reuse the fixtures from the longer
 * rollup structure due to them having the same leading structure.
 */
...
TEST_F(root_rollup_tests, test_2_real_1_padding) {
  auto tx_data = create_root_rollup_tx(
      "root_211", {{js_proofs[0], js_proofs[1]}, {js_proofs[2]}});
  auto result = verify_logic(tx_data, root_rollup_cd);
  ASSERT_TRUE(result.logic_verified);
}
...
TEST_F(root_rollup_tests, test_3_real_0_padding) {
  auto tx_data =
      create_root_rollup_tx("root_221", {{js_proofs[0], js_proofs[1]},
                                         {js_proofs[2], js_proofs[3]},
                                         {js_proofs[4]}});
  auto result = verify_logic(tx_data, root_rollup_cd);
  ASSERT_TRUE(result.logic_verified);
}
...
TEST_F(root_rollup_tests, test_incorrect_new_data_root_fails) {
  auto tx_data =
      create_root_rollup_tx("bad_new_data_root_fail", {{js_proofs[0]}});
...
TEST_F(root_rollup_tests, test_inner_rollups_out_of_order_fail) {
  auto tx_data = create_root_rollup_tx(
      "root_221", {{js_proofs[0], js_proofs[1]}, {js_proofs[2], js_proofs[3]}});
...
TEST_F(root_rollup_tests,
       test_defi_valid_previous_defi_hash_for_0_interactions) {
  auto tx_data = create_root_rollup_tx("root_1", {{js_proofs[0]}});
  auto result = verify_logic(tx_data, root_rollup_cd);
  ASSERT_TRUE(result.logic_verified);
...
TEST_F(root_rollup_tests, test_asset_ids_missing_fails) {
  ...
  tx_data.asset_ids[0] =
      tx_data.asset_ids[1]; // asset_ids = [0, aid3, aid3, aid2]
...
TEST_F(root_rollup_tests, test_asset_ids_repeating_fails) {
  auto tx_data = create_full_logic_root_rollup_tx();
  tx_data.asset_ids[1] =
      tx_data.asset_ids[0]; // asset_ids = [0, aid1, aid1, aid2]
...
TEST_F(root_rollup_tests, test_asset_ids_reordering) {
  auto tx_data = create_full_logic_root_rollup_tx();
  std::swap(tx_data.asset_ids[1],
            tx_data.asset_ids[2]); // asset_ids = [0, aid1, aid2, aid3]
...
TEST_F(root_rollup_tests, test_bridge_call_datas_missing_fails) {
  auto tx_data = create_full_logic_root_rollup_tx();
  tx_data.bridge_call_datas[0] =
      tx_data.bridge_call_datas[1]; // bridge_call_datas = [bid3, bid3, 0, 0]
...
TEST_F(root_rollup_tests, test_bridge_call_datas_repeating_fails) {
  auto tx_data = create_full_logic_root_rollup_tx();
  tx_data.bridge_call_datas[1] =
      tx_data.bridge_call_datas[0]; // bridge_call_datas = [bid2, bid2, 0, 0]
...
TEST_F(root_rollup_tests, test_bridge_call_datas_reordering) {
  ...
  std::swap(
      tx_data.bridge_call_datas[1],
      tx_data.bridge_call_datas[0]); // bridge_call_datas = [bid3, bid2, 0, 0]
...
TEST_F(root_rollup_tests, test_full_logic) {
  ...
  EXPECT_EQ(rollup_data.defi_interaction_notes[0],
            tx_data.defi_interaction_notes[0].commit());
  EXPECT_EQ(rollup_data.defi_interaction_notes[1],
            tx_data.defi_interaction_notes[1].commit());
