...
protected:
  ...
  static void SetUpTestCase() {
    ...
    tx_rollup2_cd =
        rollup::get_circuit_data(2, js_cd, account_cd, claim_cd, srs,
                                 FIXTURE_PATH, true, persist, persist);
    tx_rollup3_cd =
        rollup::get_circuit_data(3, js_cd, account_cd, claim_cd, srs,
                                 FIXTURE_PATH, true, persist, persist);
  }
  ...
  root_rollup_tx
  create_root_rollup_tx(std::string const &test_name, uint32_t rollup_id,
    ...
    for (auto txs : rollup_structure) {
      ...
      auto rollup = rollup::create_rollup_tx(
          context.world_state, tx_rollup_cd.rollup_size, txs, {}, {0});
      auto rollup_data =
          fixtures::compute_or_load_fixture(TEST_PROOFS_PATH, name, [&] {
    ...
    return root_rollup::create_root_rollup_tx(
        context.world_state, rollup_id, context.world_state.defi_tree.root(),
        old_defi_path, rollups_data, {}, {0});
  }
...
HEAVY_TEST_F(root_rollup_full_tests,
             test_root_rollup_3x2_and_detect_circuit_change) {
  static constexpr auto rollups_per_rollup = 3U;

  auto root_rollup_cd = get_circuit_data(rollups_per_rollup, tx_rollup2_cd, srs,
                                         FIXTURE_PATH, true, false, false);
  ...
  auto tx_data =
      create_root_rollup_tx("test_root_rollup_3x2", 0, tx_rollup2_cd,
                            {{js_proofs[0], js_proofs[1]}, {js_proofs[2]}});
  ...
  EXPECT_EQ(rollup_data.new_data_roots_root,
            context.world_state.root_tree.root());
  ...
  if (!(circuit_gate_count::is_circuit_change_expected)) {
    ...
    EXPECT_EQ(from_buffer<uint256_t>(vk_hash_root_rollup),
              circuit_vk_hash::ROOT_ROLLUP)
    ...
    EXPECT_LE(number_of_gates_root_rollup,
              circuit_gate_next_power_of_two::ROOT_ROLLUP -
                  waffle::ComposerBase::NUM_RESERVED_GATES)
        << "You have exceeded the next power of two limit for the root rollup "
           "circuit.";
  } else {
    EXPECT_LE(number_of_gates_root_rollup,
              circuit_gate_next_power_of_two::ROOT_ROLLUP -
...
HEAVY_TEST_F(root_rollup_full_tests, test_root_rollup_2x3) {
  ...
  auto root_rollup_cd = get_circuit_data(rollups_per_rollup, tx_rollup3_cd, srs,
                                         FIXTURE_PATH, true, false, false);
  ...
  auto tx_data = create_root_rollup_tx("test_root_rollup_2x3", 0, tx_rollup3_cd,
                                       {{js_proofs[0]}});
  auto result = verify(tx_data, root_rollup_cd);
  ASSERT_TRUE(result.verified);
...
HEAVY_TEST_F(root_rollup_full_tests, test_bad_js_proof_fails) {
  ...
  auto bad_proof = join_split::create_noop_join_split_proof(
      js_cd, context.world_state.data_tree.root(), false);

  // Our inner rollup should fail.
  auto tx_rollup_cd = tx_rollup2_cd;
  auto inner_rollup_tx = rollup::create_rollup_tx(
      context.world_state, tx_rollup_cd.rollup_size, {js_proofs[0], bad_proof});
  Composer inner_composer =
      Composer(tx_rollup_cd.proving_key, tx_rollup_cd.verification_key,
               tx_rollup_cd.num_gates);
  rollup::pad_rollup_tx(inner_rollup_tx, tx_rollup_cd.num_txs,
                        tx_rollup_cd.join_split_circuit_data.padding_proof);
  rollup::rollup_circuit(inner_composer, inner_rollup_tx,
                         tx_rollup_cd.verification_keys, tx_rollup_cd.num_txs);
  ...
  auto root_rollup_cd = get_circuit_data(rollups_per_rollup, tx_rollup_cd, srs,
                                         FIXTURE_PATH, true, false, false);
  auto root_rollup_tx = root_rollup::create_root_rollup_tx(
      context.world_state, 0, context.world_state.defi_tree.root(),
      context.world_state.defi_tree.get_hash_path(0), {inner_proof.proof_data});
  Composer root_composer =
      Composer(root_rollup_cd.proving_key, root_rollup_cd.verification_key,
               root_rollup_cd.num_gates);
  pad_root_rollup_tx(root_rollup_tx, root_rollup_cd);
  root_rollup_circuit(
      root_composer, root_rollup_tx,
