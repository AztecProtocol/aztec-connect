...
protected:
  rollup_full_tests() : context(js_cd, account_cd, claim_cd) {}
...
// Full proofs.
HEAVY_TEST_F(rollup_full_tests,
             test_1_proof_in_1_rollup_full_proof_and_detect_circuit_change) {
  ...
  auto join_split_proof = context.create_join_split_proof(
      {2, 3}, {100, 50}, {70, 110 - tx_fee}, 30, 0);
  auto rollup =
      create_rollup_tx(context.world_state, rollup_size, {join_split_proof});

  auto rollup_circuit_data = rollup::get_circuit_data(
      rollup_size, js_cd, account_cd, claim_cd, srs, "", true, false, false);
  ...
  if (!(circuit_gate_count::is_circuit_change_expected)) {
    ...
    EXPECT_LE(number_of_gates_rollup,
              circuit_gate_next_power_of_two::ROLLUP -
                  waffle::ComposerBase::NUM_RESERVED_GATES)
        << "You have exceeded the next power of two limit for the rollup "
           "circuit.";
  } else {
    EXPECT_LE(number_of_gates_rollup,
              circuit_gate_next_power_of_two::ROLLUP -
...
HEAVY_TEST_F(rollup_full_tests, test_1_proof_in_2_rollup_full_proof) {
  ...
  auto join_split_proof =
      context.create_join_split_proof({2, 3}, {100, 50}, {70, 80 - tx_fee});
  auto rollup =
      create_rollup_tx(context.world_state, rollup_size, {join_split_proof});
...
HEAVY_TEST_F(rollup_full_tests,
             test_1_js_proof_1_account_proof_in_2_rollup_full_proof) {
  size_t rollup_size = 2;
