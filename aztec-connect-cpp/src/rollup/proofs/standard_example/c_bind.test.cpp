...
TEST(client_proofs, test_standard_example_c_bindings) {
  ...
  scalar_multiplication::Pippenger pippenger(
      "../barretenberg/cpp/srs_db/ignition", 32768);
  prover->key->reference_string =
      std::make_shared<waffle::PippengerReferenceString>(&pippenger);
  ...
  transcript.open("../barretenberg/cpp/srs_db/ignition/transcript00.dat",
                  std::ifstream::binary);
  ...
  bool verified = standard_example__verify_proof(
      proof.proof_data.data(), (uint32_t)proof.proof_data.size());
