...
inline field_ct compute_nullifier(field_ct const& note_commitment)
{
    ...
    // pseudorandom-looking nullifier) because the contents of a claim note commitment are public anyway.
    ...
    // circuit (identities (public keys) are NOT leaked though). However, when those value notes are
