...
// In mock mode, mock proofs (expected public inputs, but no constraints) are
// generated.
bool mock_proofs;
// Create big circuits proving keys lazily to improve startup times.
bool lazy_init;
// True if rollup circuit data (proving and verification keys) are to be
// persisted to disk. We likely don't have enough memory to hold all keys in
// memory, and loading keys from disk is faster.
bool persist;
...
void init_tx_rollup(size_t num_txs) {
  if (tx_rollup_cd.proving_key) {
    // We always have a vk if we have a pk, as we request both in the call to
    // get_circuit_data.
    return;
  }
  if (lazy_init) {
    // In lazy init mode we conserve memory. Throw away the root rollup proving
    // key first.
    info("Purging root rollup proving key.");
    root_rollup_cd.proving_key.reset();
  }
  tx_rollup_cd = tx_rollup::get_circuit_data(
      num_txs, js_cd, account_cd, claim_cd, crs, data_path, true, persist,
      persist, true, true, mock_proofs);
}
...
bool create_tx_rollup() {
  ...
  read(std::cin, rollup);
  std::cerr << "Received tx rollup with " << rollup.num_txs << " txs."
            << std::endl;
...
void init_root_rollup(size_t num_rollups) {
  ...
  if (!tx_rollup_cd.verification_key) {
    // If we've never created the tx rollup circuit data, we won't have a vk.
    // Build it.
    init_tx_rollup(txs_per_inner);
  }
  if (lazy_init) {
    // In lazy init mode we conserve memory. Throw away the tx rollup proving
    // key first.
    info("Purging tx rollup proving key.");
    tx_rollup_cd.proving_key.reset();
  }
  root_rollup_cd = root_rollup::get_circuit_data(
      num_rollups, tx_rollup_cd, crs, data_path, true, persist, persist, true,
      true, mock_proofs);
}
...
bool create_root_rollup() {
  ...
  std::cerr << "Received root rollup with " << root_rollup.rollups.size()
            << " rollups." << std::endl;
...
void init_root_verifier() {
  ...
  if (!root_rollup_cd.verification_key) {
    // If we've never created the root rollup circuit data, we won't have a vk.
    // Build it.
    init_root_rollup(txs_per_inner);
  }
  root_verifier_cd = root_verifier::get_circuit_data(
      root_rollup_cd, crs, {root_rollup_cd.verification_key}, data_path, true,
      persist, persist, true, true, mock_proofs);
}
...
bool create_root_verifier() {
  ...
  auto tx = root_verifier::create_root_verifier_tx(root_rollup_proof_buf,
                                                   rollup_size);
...
int main(int argc, char **argv) {
  ...
  const std::string srs_path =
      (args.size() > 1) ? args[1] : "../barretenberg/cpp/srs_db/ignition";
  ...
  // Lazy init mode conserves memory by purging and recomputing tx/root proving
  // keys. If the halloumi instance is targeted to produce a specific type of
  // proof, use lazy init as it will only need to hold the pk of the specific
  // proof it creates in memory.
  //
  // Eager mode can be useful to create all the circuits up front at load time,
  // which is fine if they are not too big. It can be useful for determining to
  // total memory footprint of the process for certain circuit sizes.
  if (!lazy_init) {
    info("Running in eager init mode, all proving keys will be created once up "
         "front.");
    init_tx_rollup(txs_per_inner);
    init_root_rollup(inners_per_root);
    init_root_verifier();
  } else {
    info("Running in lazy init mode, tx rollup and root rollup proving keys "
  ...
  while (true) {
    ...
    case 100: {
      // Convert to buffer first, so when we call write we prefix the buffer
      // length.
