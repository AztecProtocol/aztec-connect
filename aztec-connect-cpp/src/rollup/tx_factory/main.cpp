...
tx_rollup::rollup_tx
create_inner_rollup(uint32_t num_txs, uint32_t rollup_size,
                    join_split::circuit_data const &join_split_circuit_data,
                    barretenberg::fr const &data_tree_root,
                    WorldState &world_state, bool mock_proofs) {
  info("Generating a ", rollup_size, " rollup with ", num_txs, " txs...");
  auto proofs = std::vector<std::vector<uint8_t>>(num_txs);
  for (size_t i = 0; i < num_txs; ++i) {
    proofs[i] = join_split::create_noop_join_split_proof(
        join_split_circuit_data, data_tree_root, true, mock_proofs);
  }
...
int main(int argc, char **argv) {
  ...
  if (args.size() < 4) {
    info("usage:\n", args[0],
         " <num_txs> <inner_rollup_size> <outer_rollup_size> "
         "<split_proofs_across_rollups> [output_file]");
    return -1;
  }
  ...
  const bool split_txns_across_rollups =
      args.size() > 4 ? args[4] == "true" : true;
  const bool mock_proofs = args.size() > 5 ? args[5] == "true" : true;
  const std::string output_file = args[6];

  auto crs = std::make_shared<waffle::DynamicFileReferenceStringFactory>(
  ...
  while (num_txs > 0) {
    auto n = split_txns_across_rollups ? (num_total_txs / outer_rollup_size)
                                       : std::min(num_txs, inner_rollup_size);
    num_txs -= n;

    auto rollup =
        create_inner_rollup(n, inner_rollup_size, join_split_circuit_data,
                            data_root, world_state, mock_proofs);
  ...
  auto root_rollup = root_rollup::create_root_rollup_tx(
      ...
      world_state.defi_tree.get_hash_path(0), rollups_data);
