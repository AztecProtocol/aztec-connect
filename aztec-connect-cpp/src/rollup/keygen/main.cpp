...
int main(int argc, char **argv) {
  ...
  if (args.size() < 4) {
    info("usage: ", args[0],
         " <num inner txs> <comma separated valid outer sizes> <output path> "
         "<mock> [srs path]");
    return 1;
  }
  ...
  const std::string srs_path =
      (args.size() > 5) ? args[5] : "../barretenberg/cpp/srs_db/ignition";

  auto srs =
      std::make_shared<waffle::DynamicFileReferenceStringFactory>(srs_path);
  ...
  if (!mock_proof) {
    ...
    auto rollup_cd = tx_rollup::get_circuit_data(
        num_inner_tx, join_split_cd, account_cd, claim_cd, srs, "", true, false,
        false, true, true);

    // Release memory held by proving key, we don't need it.
    rollup_cd.proving_key.reset();

    std::vector<std::shared_ptr<waffle::verification_key>>
        valid_root_rollup_vks;
    ...
    for (auto i : valid_outer_sizes) {
      ...
      root_rollup_cd = root_rollup::get_circuit_data(
          i, rollup_cd, srs, "", true, false, false, true, true);
      valid_root_rollup_vks.emplace_back(root_rollup_cd.verification_key);
    }

    root_verifier_cd = root_verifier::get_circuit_data(
        root_rollup_cd, srs, valid_root_rollup_vks, "", true, false, false,
        true, true);
    std::replace(outer_size.begin(), outer_size.end(), ',', '_');
    auto class_name = format(mock_proof ? "Mock" : "", "VerificationKey",
                             num_inner_tx, "x", outer_size);
    ...
    // Taking a big shortcut here. Rather than computing all the layers of
    // circuits just to get the number of public inputs, we know the root
    // verifier has 17. This is obviously a bit more brittle but is worth the
    // speed up.
