...
export const deploySyncBridge = async (owner: Signer, rollup: Contract, outputToken: Contract) => {
  ...
  if (actionTxReceipt && actionTxReceipt.status === 1) {
    console.log(`SyncBridge action successfully set in ${actionTxReceipt.transactionHash}`);
    const action = await bridge.action();
    console.log(`Action outputA ${action.outputA}`);
    console.log(`Action outputB ${action.outputB}`);
  } else {
    console.log('Setting SyncBridge action failed');
  }
