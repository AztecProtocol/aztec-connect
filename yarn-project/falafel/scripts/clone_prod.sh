...
AC_PROD_DATA_DIR='mounts/aztec-connect-prod-falafel-data/data'
...
if [ ! -d "$TARGET_DIR/world_state.db" ]; then
  scp -r bastion:$AC_PROD_DATA_DIR/world_state.db $TARGET_DIR
else
  echo "$TARGET_DIR/world_state.db already exists, skipping..."
fi

if [ ! -f "$TARGET_DIR/config" ]; then
  scp bastion:$AC_PROD_DATA_DIR/config $TARGET_DIR
else
