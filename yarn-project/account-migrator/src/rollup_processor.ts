...
import { RollupProofData } from './rollup_proof.js';
import { TxHash } from '@aztec/barretenberg/blockchain';
import { Contract, ethers } from 'ethers';
import { default as RollupABI } from './abi/RollupProcessor.json' assert { type: 'json' };
...
export class RollupProcessor {
  ...
  constructor(rollupContractAddress: EthAddress, private provider: Web3Provider) {
    this.rollupProcessor = new ethers.Contract(rollupContractAddress.toString(), RollupABI.abi, this.provider);
  }
  ...
  private decodeBlock(tx: TransactionResponse, receipt: TransactionReceipt): Block {
    const rollupAbi = new ethers.utils.Interface(RollupABI.abi);
