...
export interface SpendingKeyAccountJson {
...
export function spendingKeyAccountToJson(account: SpendingKeyAccount): SpendingKeyAccountJson {
  return {
    spendingPublicKey: account.spendingPublicKey.toString(),
    aliasHash: account.aliasHash.toString(),
    accountIndex: account.accountIndex,
    accountPath: account.accountPath.toBuffer().toString('base64'),
  };
}

export function spendingKeyAccountFromJson(account: SpendingKeyAccountJson): SpendingKeyAccount {
  return {
    spendingPublicKey: GrumpkinAddress.fromString(account.spendingPublicKey),
    aliasHash: AliasHash.fromString(account.aliasHash),
    accountIndex: account.accountIndex,
    accountPath: HashPath.fromBuffer(Buffer.from(account.accountPath, 'base64')),
  };
}
