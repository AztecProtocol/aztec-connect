...
export class DepositHandler {
  private publicInput: AssetValue;
  protected depositProofOutput?: ProofOutput;
  protected depositSignature?: Buffer;
  ...
  constructor(
    ...
    if (assetValue.assetId !== fee.assetId && assetValue.value && fee.value) {
      throw new Error('Inconsistent asset id.');
    }

    const assetId = assetValue.value ? assetValue.assetId : fee.assetId;
    if (!blockchain.getAsset(assetId)) {
      throw new Error('Unsupported asset.');
    }

    this.publicInput = { assetId, value: assetValue.value + fee.value };
  }
  ...
  public async sign() {
    ...
    this.depositSignature = await ethSigner.signPersonalMessage(signingData, this.depositor);
    return this.depositSignature;
  }

  public isSignatureValid() {
    if (!this.depositProofOutput) {
      throw new Error('Call createProofs() and sign() first.');
    }
    if (!this.depositSignature) {
      throw new Error('Call sign() first.');
    }

    const signingData = this.getSigningData();
    return validateSignature(this.depositor, this.depositSignature, signingData);
  }
