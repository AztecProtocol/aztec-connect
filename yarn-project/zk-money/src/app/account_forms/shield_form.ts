import { CutdownAsset } from '../types.js';
import {
  AztecSdk,
  DepositController,
  EthAddress,
...
import { isSameAlias, isValidAliasInput } from '../alias.js';
import { EthAccount, EthAccountEvent, EthAccountState } from '../eth_account/index.js';
import {
  BigIntValue,
  BoolInput,
  clearMessage,
...
import { KeyVault } from '../key_vault.js';
import { Network } from '../networks.js';
import { Provider, ProviderStatus } from '../provider.js';
import { RollupService, RollupServiceEvent, RollupStatus, TxFee } from '../rollup_service.js';
import { formatBaseUnits, max, min, toBaseUnits } from '../units.js';
import { AccountForm, AccountFormEvent } from './account_form.js';
  ...
  private async shield() {
    ...
    if (this.status <= ShieldStatus.APPROVE_PROOF) {
      ...
      if (signingData && !this.proof.signed && !this.isContract) {
        ...
        this.prompt(`Please sign the message in your wallet containing the following transaction ID: 0x${digest}`);
        try {
