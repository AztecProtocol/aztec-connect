...
import { CoreSdkServerStub } from '../../core_sdk/core_sdk_server_stub.js';
...
describe('caramel core sdk', () => {
  ...
  beforeEach(() => {
    ...
    core = {
      ...
      removeUser: jest.fn(),
      userExists: jest.fn<any>().mockResolvedValue(true),
      derivePublicKey: jest.fn((privateKey: Buffer) => {
        const user = users.find(u => u.accountPrivateKey.toString() === privateKey.toString());
        return user?.id || GrumpkinAddress.random().toString();
      }),
      getBalance: jest.fn<any>().mockResolvedValue('0'),
    } as any;
