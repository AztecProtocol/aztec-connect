...
const TRANCHE_BYTECODE_HASH = Buffer.from(
...
const parseTranches = () => {
  ...
  const poolSpecs = ASSETS.flatMap(asset => {
    ...
    return assetTranches.filter((tranche: any) => (tranche.expiration * 1000) > expiryCutOff).map((tranche: any) => {
      return {
        asset,
...
export const deployElementBridge = async (owner: Signer, rollupAddress: string, trancheFactoryAddress: string, trancheByteCode: Buffer, balancerAddress: string) => {
  ...
  console.error(`Element contract address: ${bridge.address}`);
  return bridge;
};
