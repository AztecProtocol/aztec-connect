...
library Bn254Crypto {
    ...
    function new_g1(uint256 x, uint256 y)
        internal
        pure
        returns (StandardTypes.G1Point memory)
    {
        uint256 xValue;
        uint256 yValue;
        assembly {
        ...
        return StandardTypes.G1Point(xValue, yValue);
    }

    function new_g2(uint256 x0, uint256 x1, uint256 y0, uint256 y1)
        internal
        pure
        returns (StandardTypes.G2Point memory)
    {
        return StandardTypes.G2Point(x0, x1, y0, y1);
    }

    function P1() internal pure returns (StandardTypes.G1Point memory) {
        return StandardTypes.G1Point(1, 2);
    }

    function P2() internal pure returns (StandardTypes.G2Point memory) {
        return StandardTypes.G2Point({
    ...
    function pairingProd2(
        StandardTypes.G1Point memory a1,
        StandardTypes.G2Point memory a2,
        StandardTypes.G1Point memory b1,
        StandardTypes.G2Point memory b2
    ...
    *   y^2 = x^3 + 3 mod p
    */
    function validateG1Point(StandardTypes.G1Point memory point) internal pure {
        bool is_well_formed;
        uint256 p = p_mod;
        assembly {
            let x := mload(point)
