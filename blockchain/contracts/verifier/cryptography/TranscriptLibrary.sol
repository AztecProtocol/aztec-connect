...
library TranscriptLibrary {
    ...
    {
        bytes memory formatted_circuit_size = format_4_byte_variable(
            uint32(circuit_size)
        );
        bytes memory formatted_num_public_inputs = format_4_byte_variable(
            uint32(num_public_inputs)
        );

        // transcript.debug_data = abi.encodePacked(
        //     formatted_circuit_size,
        //     formatted_num_public_inputs
        // );
        transcript.current_challenge = keccak256(
            abi.encodePacked(
                formatted_circuit_size,
                formatted_num_public_inputs
            )
        ...
        // emit ChallengeDebug(
        //     bytes32(uint256(transcript.current_challenge) % r_mod)
        // );
        transcript.data = abi.encodePacked(transcript.current_challenge);
        transcript.challenge_counter = 0;
    }

    function format_4_byte_variable(uint32 input)
        internal
        pure
    ...
    // function get_challenge_without_reduction(Transfript memory self)
    //     internal
    //     pure
    //     returns (Types.Fr memory)
    // {
