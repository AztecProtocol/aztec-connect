...
contract Decoder {
    ...
    function depositTx(uint inPtr, uint outPtr) internal pure returns (uint) {
        assembly {
    ...
    function withdrawTx(uint inPtr, uint outPtr) internal pure returns (uint) {
        assembly {
    ...
    function sendTx(uint inPtr, uint outPtr) internal pure returns (uint) {
        assembly {
    ...
    function accountTx(uint inPtr, uint outPtr) internal pure returns (uint) {
        assembly {
            // Account
    ...
    function defiDepositTx(uint inPtr, uint outPtr) internal pure returns (uint) {
        assembly {
            // Defi Deposit
    ...
    function defiClaimTx(uint inPtr, uint outPtr) internal pure returns (uint) {
        assembly {
            // Defi Claim
        ...
        returns (bytes memory proofData, uint256 numTxs, uint256 publicInputsHash)
    {
        ...
        uint256 tailInPtr;
        uint256 txSize;
        function(uint, uint) pure returns (uint) callfunc;
        ...
        }
        {
            ...
            function(uint, uint) pure returns (uint) t1 = depositTx;
            function(uint, uint) pure returns (uint) t2 = withdrawTx;
            function(uint, uint) pure returns (uint) t3 = sendTx;
        ...
        assembly {
            ...
            let encodedInnerDataSize := and(calldataload(add(inPtr, sub(rollupHeaderInputLength, 0x1c))), 0xffffffff)
