...
resource "aws_s3_bucket" "docs" {
  bucket = "${var.DEPLOY_TAG}-docs.aztec.network"
  acl    = "public-read"
  ...
  "Statement": [
    {
      ...
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::${var.DEPLOY_TAG}-docs.aztec.network/*"
    }
  ]
}
EOF
...
resource "aws_cloudfront_distribution" "docs_distribution" {
  ...
  aliases = ["${var.DEPLOY_TAG}-docs.aztec.network", "docs.aztec.network"]
