...
export function computeAccountAliasIdNullifier(accountAliasId: AccountAliasId, pedersen: Pedersen) {
  ...
  // to ensure we use the same generators in Pedersen hashing.
  const accountAliasIdIndex = 10;
  const proofId = 1;
  const prefixBuf = numToUInt32BE(proofId, 32);
  return pedersen.compressWithHashIndex(
    [prefixBuf, accountAliasId.toBuffer(), Buffer.alloc(32, 0)],
    accountAliasIdIndex,
  );
}
