...
const debug = createDebug('bb:unrolled_prover');
...
export class UnrolledProver {
  ...
  public async createProof(proverPtr: number) {
    ...
    await this.wasm.call('unrolled_prover_execute_preamble_round', proverPtr);
    timer.mark('preamble end');
    await this.processProverQueue(proverPtr, circuitSize);
    timer.mark('first round start');
    await this.wasm.call('unrolled_prover_execute_first_round', proverPtr);
    timer.mark('first round end');
    await this.processProverQueue(proverPtr, circuitSize);
    timer.mark('second round start');
    await this.wasm.call('unrolled_prover_execute_second_round', proverPtr);
    timer.mark('second round end');
    await this.processProverQueue(proverPtr, circuitSize);
    timer.mark('third round start');
    await this.wasm.call('unrolled_prover_execute_third_round', proverPtr);
    timer.mark('third round end');
    await this.processProverQueue(proverPtr, circuitSize);
    timer.mark('fourth round start');
    await this.wasm.call('unrolled_prover_execute_fourth_round', proverPtr);
    timer.mark('fourth round end');
    await this.processProverQueue(proverPtr, circuitSize);
    timer.mark('fifth round start');
    await this.wasm.call('unrolled_prover_execute_fifth_round', proverPtr);
    timer.mark('fifth round end');
    timer.mark('sixth round start');
    await this.wasm.call('unrolled_prover_execute_sixth_round', proverPtr);
    timer.mark('sixth round end');
    await this.processProverQueue(proverPtr, circuitSize);
    timer.mark('done');
    const proofSize = await this.wasm.call('unrolled_prover_export_proof', proverPtr, 0);
  ...
  private async processProverQueue(proverPtr: number, circuitSize: number) {
    await this.wasm.call('unrolled_prover_get_work_queue_item_info', proverPtr, 0);
    ...
    for (let i = 0; i < scalarJobs; ++i) {
      const scalarsPtr = await this.wasm.call('unrolled_prover_get_scalar_multiplication_data', proverPtr, i);
    ...
    for (let i = 0; i < fftJobs; ++i) {
      const coeffsPtr = await this.wasm.call('unrolled_prover_get_fft_data', proverPtr, 0, i);
    ...
    for (let i = 0; i < ifftJobs; ++i) {
      const coeffsPtr = await this.wasm.call('unrolled_prover_get_ifft_data', proverPtr, i);
      const coefficients = await this.wasm.sliceMemory(coeffsPtr, coeffsPtr + circuitSize * 32);
  ...
  private async doFft(
    ...
    await this.wasm.call('unrolled_prover_put_fft_data', proverPtr, resultPtr, i);
  ...
  private async doIfft(proverPtr: number, i: number, circuitSize: number, coefficients: Uint8Array) {
    ...
    const resultPtr = await this.wasm.call('bbmalloc', circuitSize * 32);
    await this.wasm.transferToHeap(result, resultPtr);
    await this.wasm.call('unrolled_prover_put_ifft_data', proverPtr, resultPtr, i);
    await this.wasm.call('bbfree', resultPtr);
  }
}
