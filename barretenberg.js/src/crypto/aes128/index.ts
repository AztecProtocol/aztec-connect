...
export class Aes128 {
  ...
  public encryptBufferCBC(data: Uint8Array, iv: Uint8Array, key: Uint8Array) {
    ...
    const result: Buffer = Buffer.from(this.wasm.sliceMemory(mem + input.length + key.length + iv.length, mem + input.length + key.length + iv.length + input.length));
  ...
  public decryptBufferCBC(data: Uint8Array, iv: Uint8Array, key: Uint8Array) {
    ...
    const result: Buffer = Buffer.from(this.wasm.sliceMemory(mem + data.length + key.length + iv.length, mem + data.length + key.length + iv.length + data.length));
    this.wasm.call('bbfree', mem);
    return result;
  }
}
