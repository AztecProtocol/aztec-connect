...
export async function getSpendingKeysFromGenesisData(
  accountPublicKeys: GrumpkinAddress[],
  accountsData: Buffer,
  hasher: Hasher,
  rollupSize: number,
) {
  ...
  // extract the spending keys and find those for the given accountPublicKeys
  const aliasesAndKeys = parseGenesisAliasesAndKeys(accounts);
  const spendingKeys = aliasesAndKeys.spendingKeys.filter(key =>
    accountPublicKeys.some(accountPublicKey => accountPublicKey.equals(key.accountPublicKey)),
  );
