...
export const joinSplitTxInputToJson = ({
  ...
  claimNote: claimNote.toBuffer().toString('base64'),
  spendingPublicKey: spendingPublicKey.toString(),
  aliasHash: aliasHash.toString(),
  accountPath: accountPath.toBuffer().toString('base64'),
  dataRoot: dataRoot.toString('base64'),
  backwardLink: backwardLink.toString('base64'),
  ...rest,
});
...
export const joinSplitTxInputFromJson = ({
  ...
  claimNote: ClaimNoteTxData.fromBuffer(Buffer.from(claimNote, 'base64')),
  spendingPublicKey: GrumpkinAddress.fromString(spendingPublicKey),
  aliasHash: AliasHash.fromString(aliasHash),
  accountPath: HashPath.fromBuffer(Buffer.from(accountPath, 'base64')),
  dataRoot: Buffer.from(dataRoot, 'base64'),
  backwardLink: Buffer.from(backwardLink, 'base64'),
  ...rest,
});
