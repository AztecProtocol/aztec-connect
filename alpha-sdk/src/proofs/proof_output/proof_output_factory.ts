    ...
    return await this.serialQueue.push(async () => {
      ...
      for (let i = 0; i < proofInputs.length; ++i) {
        ...
        if (proofInput.type === ProofInputType.DefiProofInput) {
          proofOutputs.push(await this.createDefiProof(proofInput, signature, txRefNo, keyPair, timeout));
        } else if (proofInput.type === ProofInputType.PaymentProofInput) {
          proofOutputs.push(await this.createPaymentProof(proofInput, signature, txRefNo, keyPair, timeout));
        } else {
          proofOutputs.push(await this.createAccountProof(proofInput, signature, txRefNo, timeout));
        }
      }
      return proofOutputs;
    });
