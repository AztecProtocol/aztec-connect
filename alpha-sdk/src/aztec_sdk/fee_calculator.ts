...
export class FeeCalculator {
  ...
  public async getDefiFees(
    ...
    const { accountPublicKey, spendingKeyRequired, assetValue, feeSignificantFigures = 0 } = options;
    ...
    if (!accountPublicKey) {
      throw new Error(`'accountPublicKey' not provided.`);
    }
    ...
    if (bridgeCallData.inputAssetIdB !== undefined) {
      const noteB = await this.core.pickNote(accountPublicKey, bridgeCallData.inputAssetIdB, depositValue, {
        spendingKeyRequired,
        excludePendingNotes,
      });
      if (noteB?.value === depositValue) {
        ...
        const notes = await this.core.pickNotes(accountPublicKey, bridgeCallData.inputAssetIdB, depositValue, {
          spendingKeyRequired,
          excludePendingNotes,
        });
        ...
        if (feeAssetId !== assetId) {
          const noteA = await this.core.pickNote(accountPublicKey, assetId, depositValue, {
            spendingKeyRequired,
            excludePendingNotes,
          });
          ...
          return await Promise.all(
            fees.map(async fee => {
              ...
              const noteA = await this.core.pickNote(accountPublicKey, assetId, noteValueA, {
                spendingKeyRequired,
                excludePendingNotes,
              });
    ...
    const getNumMerges = async (valueA: bigint) => {
      const notes = await this.core.pickNotes(accountPublicKey, assetId, valueA, {
        spendingKeyRequired,
        excludePendingNotes,
      });
      if (!notes.length) {
      ...
      if (bridgeCallData.numInputAssets === 2) {
        const note = await this.core.pickNote(accountPublicKey, assetId, valueA, {
          spendingKeyRequired,
          excludePendingNotes,
        });
    ...
    if (feeAssetId !== assetId) {
      ...
      const spendableValue = await this.core.getSpendableSum(accountPublicKey, assetId, {
        spendingKeyRequired,
        excludePendingNotes,
      });
      const getChainedTxsFee = async (baseFee: bigint) => {
        ...
        const numNotes =
          1 +
          (
            await this.core.pickNotes(accountPublicKey, assetId, spendableValue, {
              spendingKeyRequired,
              excludePendingNotes,
            })
          ).length;
  ...
  private async getTransactionFees(
    ...
    const { accountPublicKey, spendingKeyRequired, assetValue, excludePendingNotes } = options;
    const txFees = await this.core.getTxFees(assetId);
    const [transferFee] = txFees[TxType.TRANSFER];
    ...
    const getNumNotes = async (privateInput: AssetValue) => {
      const notes = await this.core.pickNotes(accountPublicKey, privateInput.assetId, privateInput.value, {
        spendingKeyRequired,
        excludePendingNotes,
      });
      return notes.length;
  ...
  private async getChainedTxsFee(
    ...
    const { accountPublicKey, spendingKeyRequired, excludePendingNotes, feeSignificantFigures = 0 } = options;
    if (!accountPublicKey) {
