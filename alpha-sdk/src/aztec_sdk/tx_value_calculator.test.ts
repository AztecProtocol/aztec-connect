...
describe('tx value calculator', () => {
  ...
  const mockNotePicker = (notesA: MockNote[], notesB = notesA, noneFeePayingNotes = notesA) => {
    ...
    core.getSpendableNoteValues.mockImplementation((_, aid, options) =>
      getNotePicker(aid).getSpendableNoteValues(options),
    );
    core.getSpendableSum.mockImplementation((_, aid, options) =>
      getNotePicker(aid)
        .getSpendableNoteValues(options)
        .reduce((sum, v) => sum + v, 0n),
    );
    core.getMaxSpendableNoteValues.mockImplementation((_, aid, options) =>
      getNotePicker(aid).getMaxSpendableNoteValues(options),
    );
    core.pickNotes.mockImplementation((_, aid, value, options) => getNotePicker(aid).pick(value, options));
    core.pickNote.mockImplementation((_, aid, value, options) => getNotePicker(aid).pickOne(value, options));
  };
  ...
  describe('getMaxWithdrawValue', () => {
    it('return max value for withdrawal', async () => {
      {
        expect(await txValueCalculator.getMaxWithdrawValue(accountPublicKey, assetId)).toEqual({
          assetId,
          value: 0n,
      ...
      }
      {
        ...
        expect(
          await txValueCalculator.getMaxWithdrawValue(accountPublicKey, assetId, { feeSignificantFigures: 2 }),
        ).toEqual({
          assetId,
    ...
    it('return correct max value for withdrawing to normal account', async () => {
      ...
      const recipient = EthAddress.random();
      {
        ...
        expect(await txValueCalculator.getMaxWithdrawValue(accountPublicKey, assetId, { recipient })).toEqual({
          assetId,
          value: 150n + 130n + 120n - 106n - 105n,
          fee: { assetId, value: 106n + 105n }, // 211n
        });
        expect(
          await txValueCalculator.getMaxWithdrawValue(accountPublicKey, assetId, {
            recipient,
    ...
    it('return correct max value for faster settlement time', async () => {
      ...
      {
        expect(await txValueCalculator.getMaxWithdrawValue(accountPublicKey, assetId, { txSettlementTime })).toEqual({
          assetId,
    ...
    it('create number of merges based on available fee', async () => {
      const notesA = [150n, 140n, 130n, 120n, 100n];
      {
        ...
        expect(await txValueCalculator.getMaxWithdrawValue(accountPublicKey, noneFeePayingAssetId)).toEqual({
      ...
      }
      {
        ...
        expect(
          await txValueCalculator.getMaxWithdrawValue(accountPublicKey, noneFeePayingAssetId, {
            feeSignificantFigures: 2,
          }),
        ).toEqual({
          assetId: noneFeePayingAssetId,
  ...
  describe('getMaxTransferValue', () => {
    it('return max value for transfer', async () => {
      {
        expect(await txValueCalculator.getMaxTransferValue(accountPublicKey, assetId)).toEqual({
          assetId,
          value: 0n,
          fee: { assetId, value: 0n },
        });
      ...
      }
      {
        ...
        expect(
          await txValueCalculator.getMaxTransferValue(accountPublicKey, assetId, { feeSignificantFigures: 2 }),
        ).toEqual({
          assetId,
    ...
    it('return correct max value for faster settlement time', async () => {
      ...
      const txSettlementTime = TxSettlementTime.INSTANT;
      {
        expect(await txValueCalculator.getMaxTransferValue(accountPublicKey, assetId, { txSettlementTime })).toEqual({
          assetId,
          value: 150n + 130n + 120n - 207n - 105n,
          fee: { assetId, value: 207n + 105n }, // 312n
        });
        expect(
          await txValueCalculator.getMaxTransferValue(accountPublicKey, assetId, {
            txSettlementTime,
            feeSignificantFigures: 2,
          }),
        ).toEqual({
    ...
    it('create number of merges based on available fee', async () => {
      ...
      {
        core.getMaxSpendableNoteValues.mockImplementation((_, aid) => (aid === noneFeePayingAssetId ? notesA : []));
        expect(await txValueCalculator.getMaxTransferValue(accountPublicKey, noneFeePayingAssetId)).toEqual({
          assetId: noneFeePayingAssetId,
          value: 0n,
          fee: { assetId, value: 0n },
        });
      ...
      }
      {
        ...
        expect(
          await txValueCalculator.getMaxTransferValue(accountPublicKey, noneFeePayingAssetId, {
            feeSignificantFigures: 2,
          }),
        ).toEqual({
          assetId: noneFeePayingAssetId,
  ...
  describe('getMaxDefiValue', () => {
    ...
    it('return max value for defi deposit', async () => {
      {
        expect(await txValueCalculator.getMaxDefiValue(accountPublicKey, bridgeCallData)).toEqual({
          assetId,
          value: 0n,
          fee: { assetId, value: 0n },
        });
      ...
      }
      {
        ...
        expect(
          await txValueCalculator.getMaxDefiValue(accountPublicKey, bridgeCallData, { feeSignificantFigures: 2 }),
        ).toEqual({
          assetId,
    ...
    it('return correct max value for faster settlement time', async () => {
      core.getMaxSpendableNoteValues.mockResolvedValue([250n, 130n, 120n]);
      {
        ...
        expect(await txValueCalculator.getMaxDefiValue(accountPublicKey, bridgeCallData, { txSettlementTime })).toEqual(
          {
            assetId,
      ...
      }
      {
        ...
        expect(
          await txValueCalculator.getMaxDefiValue(accountPublicKey, bridgeCallData, {
            txSettlementTime,
            feeSignificantFigures: 2,
          }),
        ).toEqual({
