...
case $VERSION_TAG in
  testnet)
    ...
    export PRIVATE_KEY=$TF_VAR_TEST_CONTRACTS_DEPLOYER_PRIVATE_KEY
    export DEPLOYER_ADDRESS=$TF_VAR_TEST_CONTRACTS_DEPLOYER_ADDRESS
    export FAUCET_CONTROLLER=$TF_VAR_TEST_FAUCET_OPERATOR_ADDRESS
    ;;
  dev)
    export ETHEREUM_HOST=https://aztec-connect-$VERSION_TAG-mainnet-fork.aztec.network:8545/$DEV_FORK_API_KEY
    export PRIVATE_KEY=$TF_VAR_DEV_CONTRACTS_DEPLOYER_PRIVATE_KEY
    export DEPLOYER_ADDRESS=$TF_VAR_DEV_CONTRACTS_DEPLOYER_ADDRESS
    export FAUCET_CONTROLLER=$TF_VAR_DEV_FAUCET_OPERATOR_ADDRESS
    ;;
  *)
