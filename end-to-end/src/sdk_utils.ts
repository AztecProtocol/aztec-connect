...
export async function depositTokensToAztec (
  ...
  const signer = sdk.createSchnorrSigner(provider.getPrivateKeyForAddress(usersEthereumAddress)!);
...
export async function sendTokens (
  ...
  const tokenAssetId = sdk.getAssetIdByAddress(token);
  const senderSigner = sdk.createSchnorrSigner(sdk.getUserData(sender).privateKey);
...
export async function withdrawTokens (
  ...
  const tokenAssetId = sdk.getAssetIdByAddress(token);
  const withdrawSigner = sdk.createSchnorrSigner(sdk.getUserData(user).privateKey);
...
export async function defiDepositTokens(
  ...
  const tokenAssetId = sdk.getAssetIdByAddress(token);
  const defiSigner = sdk.createSchnorrSigner(sdk.getUserData(user).privateKey);
  const tokenDepositFee = (await sdk.getDefiFees(bridgeId, false, user, tokenQuantity))[settlementTime];
  const tokenAssetValue = { assetId: tokenAssetId, value: tokenQuantity };
  const defiDepositController = sdk.createDefiController(user,
    defiSigner,
    bridgeId,
    tokenAssetValue,
    tokenDepositFee
